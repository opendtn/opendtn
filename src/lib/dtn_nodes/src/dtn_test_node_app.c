...
static bool cb_bundle_send_request(dtn_test_node_app *self, int socket,
...
static bool cb_app_bundle_send_request(void *userdata, int socket,
                                       dtn_item *msg) {
...
static bool register_app_callback(dtn_test_node_app *self) {
    ...
    if (!dtn_app_register(self->app, "bundle_send_request",
                          cb_app_bundle_send_request, self))
        goto error;
...
dtn_test_node_app *dtn_test_node_app_create(dtn_test_node_app_config config) {
    ...
    if (!dtn_webserver_register_close(self->config.server, self, cb_close))
        goto error;
...
dtn_test_node_app *dtn_test_node_app_free(dtn_test_node_app *self) {
    ...
    if (!dtn_test_node_app_cast(self))
        return self;
...
dtn_test_node_app_config_from_item(const dtn_item *input) {
...
bool dtn_test_node_enable_ip_interfaces(dtn_test_node_app *self,
    ...
    return dtn_test_node_core_enable_ip_interfaces(self->core, input);
}

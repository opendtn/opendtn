...
static bool cb_login(dtn_file_node_app *self, int socket, const dtn_item *msg,
    ...
    if (!self || socket < 1 || !msg || !out)
        goto error;

    const char *pass =
        dtn_item_get_string(dtn_item_get(msg, "/parameter/password"));

    answer = dtn_event_message_create_response(msg);

    if (!pass) {

        dtn_event_set_error(answer, DTN_EVENT_ERROR_CODE_INPUT,
                            DTN_EVENT_ERROR_DESC_INPUT);

    } else if (!dtn_password_check(pass, self->config.password)) {

        dtn_event_set_error(answer, DTN_EVENT_ERROR_CODE_AUTH,
                            DTN_EVENT_ERROR_DESC_AUTH);
...
static bool cb_load_routes(dtn_file_node_app *self, int socket,
    ...
    const char *path =
        dtn_item_get_string(dtn_item_get(msg, "/parameter/path"));
    ...
    if (!path) {
        ...
        if (!dtn_file_node_core_enable_routes(self->core, path)) {
            ...
            dtn_event_set_error(answer, DTN_EVENT_ERROR_CODE_PROCESSING,
...
static bool cb_send_file(dtn_file_node_app *self, int socket,
                         const dtn_item *msg, dtn_item **out) {
    ...
    const char *source =
        dtn_item_get_string(dtn_item_get(msg, "/parameter/path/source"));

    const char *uri = dtn_item_get_string(dtn_item_get(msg, "/parameter/uri"));

    const char *dest =
        dtn_item_get_string(dtn_item_get(msg, "/parameter/path/destination"));
    ...
    if (!source || !dest || !uri) {
        ...
        bool result =
            dtn_file_node_core_send_file(self->core, uri, source, dest);

        if (!result)
            dtn_event_set_error(answer, DTN_EVENT_ERROR_CODE_SEND,
                                DTN_EVENT_ERROR_DESC_SEND);
    }
...
static bool cb_shutdown(dtn_file_node_app *self, int socket,
                        const dtn_item *msg, dtn_item **out) {
...
static bool cb_app_generic(void *userdata, int socket, dtn_item *msg,
    ...
    if (out)
        dtn_app_send_json(self->app, socket, out);
...
static bool register_app_callback(dtn_file_node_app *self) {
    ...
    if (!dtn_app_register(self->app, "login", cb_app_login, self))
        goto error;

    if (!dtn_app_register(self->app, "load_routes", cb_app_load_routes, self))
        goto error;

    if (!dtn_app_register(self->app, "send_file", cb_app_send_file, self))
        goto error;

    if (!dtn_app_register(self->app, "shutdown", cb_app_shutdown, self))
        goto error;
...
static bool init_config(dtn_file_node_app_config *config) {
    ...
    if (0 == config->name[0])
        strncpy(config->name, "localhost", 10);
...
dtn_file_node_app *dtn_file_node_app_create(dtn_file_node_app_config config) {
    ...
    dtn_socket_item_config conn =
        (dtn_socket_item_config){.loop = config.loop,
                                 .limits.threadlock_timeout_usec =
    ...
    if (-1 == self->socket) {
        ...
        dtn_log_info("created C&C listener at %s:%i", self->config.socket.host,
                     self->config.socket.port);
    }

    if (!register_app_callback(self))
        goto error;
    dtn_app_set_debug(self->app, false);

    if (!dtn_app_register_close(self->app, self, cb_close))
        goto error;
    ...
    dtn_file_node_core_config core = (dtn_file_node_core_config){
        ...
        .limits.buffer_time_cleanup_usecs =
            config.limits.buffer_time_cleanup_usecs,
...
dtn_file_node_app *dtn_file_node_app_free(dtn_file_node_app *self) {
    ...
    if (!dtn_file_node_app_cast(self))
        return self;
...
dtn_file_node_app_config_from_item(const dtn_item *input) {
    ...
    config.limits.threadlock_timeout_usec =
        dtn_item_get_number(dtn_item_get(conf, "threadlock_timeout_usec"));

    config.limits.message_queue_capacity =
        dtn_item_get_number(dtn_item_get(conf, "message_queue_capacity"));

    config.limits.link_check =
        dtn_item_get_number(dtn_item_get(conf, "link_check_usec"));

    config.limits.threads = dtn_item_get_number(dtn_item_get(conf, "threads"));

    config.limits.buffer_time_cleanup_usecs =
        dtn_item_get_number(dtn_item_get(conf, "buffer_time_cleanup_usecs"));

    config.limits.max_buffer_time_secs =
        dtn_item_get_number(dtn_item_get(conf, "max_buffer_time_secs"));

    config.limits.history_secs =
        dtn_item_get_number(dtn_item_get(conf, "history_secs"));

    const dtn_item *cbor = dtn_item_get(conf, "/cbor");

    config.limits.cbor.string_size =
        dtn_item_get_number(dtn_item_get(cbor, "string_size"));

    config.limits.cbor.utf8_string_size =
        dtn_item_get_number(dtn_item_get(cbor, "utf8_string_size"));

    config.limits.cbor.array_size =
        dtn_item_get_number(dtn_item_get(cbor, "array_size"));

    config.limits.cbor.undef_length_array =
        dtn_item_get_number(dtn_item_get(cbor, "undef_length_array"));

    config.limits.cbor.map_size =
        dtn_item_get_number(dtn_item_get(cbor, "map_size"));

    config.limits.cbor.undef_length_map =
        dtn_item_get_number(dtn_item_get(cbor, "undef_length_map"));

    config.socket =
        dtn_socket_configuration_from_item(dtn_item_object_get(conf, "socket"));

    config.password =
        dtn_password_from_item(dtn_item_object_get(conf, "password"));

    const char *str = dtn_item_get_string(dtn_item_get(conf, "/name"));
    if (str)
        strncpy(config.name, str, PATH_MAX);

    str = dtn_item_get_string(dtn_item_get(conf, "/path"));
    if (str)
        strncpy(config.path, str, PATH_MAX);

    str = dtn_item_get_string(dtn_item_get(conf, "/uri"));
    if (str)
        strncpy(config.uri, str, PATH_MAX);

    str = dtn_item_get_string(dtn_item_get(conf, "/keys"));
    if (str)
        strncpy(config.keys, str, PATH_MAX);
...
bool dtn_file_node_app_enable_ip_interfaces(dtn_file_node_app *self,
    ...
    return dtn_file_node_core_enable_ip_interfaces(self->core, input);
}

/*---------------------------------------------------------------------------*/

bool dtn_file_node_app_enable_routes(dtn_file_node_app *self,
                                     const char *path) {

    return dtn_file_node_core_enable_routes(self->core, path);
}
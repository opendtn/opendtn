...
dtn_file_node_core *dtn_file_node_core_create(dtn_file_node_core_config config){
    ...
    self->buffer = dtn_bundle_buffer_create((dtn_bundle_buffer_config){
        ...
        .limits.max_buffer_time_usecs = self->config.limits.max_buffer_time_usecs,
...
static dtn_interface_ip *get_interface_for_uri(dtn_file_node_core *self,
    const char *uri,
    dtn_socket_configuration remote){
...
static dtn_socket_configuration get_socket_config_for_uri(
    dtn_file_node_core *self, const char *uri){
...
static dtn_interface_ip *get_default_interface(dtn_file_node_core *self){
...
static dtn_list *create_bundles_for_file(dtn_file_node_core *self,
    const char *uri, const char *path, uint8_t *buffer, size_t size){
    ...
    if (path[0] == '/'){
        ...
        snprintf(destination, 2 *PATH_MAX, "dtn://%s%s", uri, path);
...
bool dtn_file_node_core_send_file(
    ...
    queue = create_bundles_for_file(self, uri, dest_path, buffer, size);
    if (!queue) goto error;
    ...
    dtn_log_debug("Send file from %s to %s", source_path, dest_path);

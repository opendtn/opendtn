...
static bool cb_load_routes(dtn_tunnel_app *self, int socket,
...
dtn_tunnel_app *dtn_tunnel_app_create(dtn_tunnel_app_config config) {
    ...
    if (0 != self->config.destination_uri[0])
        dtn_tunnel_core_set_destination_uri(self->core,
                                            self->config.destination_uri);
...
dtn_tunnel_app_config dtn_tunnel_app_config_from_item(const dtn_item *input) {
    ...
    if (str)
        strncpy(config.destination_uri, str, PATH_MAX);
...
bool dtn_tunnel_app_enable_ip_interfaces(dtn_tunnel_app *self,
    ...
    return dtn_tunnel_core_enable_ip_interfaces(self->core, input);
}
...
bool dtn_tunnel_app_enable_routes(dtn_tunnel_app *self, const char *path) {
    ...
    return dtn_tunnel_core_enable_routes(self->core, path);
}
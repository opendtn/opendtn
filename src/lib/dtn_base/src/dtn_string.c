...
size_t dtn_string_len(char const *str) {
...
char const *dtn_string_chr(char const *str, char c) {
...
char *dtn_string_copy(char *restrict target, char const *restrict source,
  ...
  if ((0 == target) && (0 == max_len)) {
    return dtn_string_dup(source);
  }

  if (0 == target) {
    size_t len = dtn_string_len(source);
    if (len > max_len) {
      len = max_len;
    }
...
char *dtn_string_dup(char const *to_dup) {

  if (0 == to_dup)
    goto error;

  size_t len = strnlen(to_dup, dtn_STRING_DEFAULT_SIZE);

  if ((dtn_STRING_DEFAULT_SIZE < len + 1) || (0 != to_dup[len])) {
...
int dtn_string_compare(char const *str1, char const *str2) {

  if ((0 == str1) || (0 == str2)) {
    return (str1 == str2) ? 0 : -1;
  } else {
    return strncmp(str1, str2, dtn_STRING_DEFAULT_SIZE);
  }
}

/*----------------------------------------------------------------------------*/

int dtn_string_compare_case_insensitive(char const *str1, char const *str2) {
...
bool dtn_string_equal(char const *str1, char const *str2) {
...
bool dtn_string_equal_nocase(char const *str1, char const *str2) {
...
bool dtn_string_startswith(char const *str, char const *prefix) {

  if (dtn_ptr_valid(prefix, "No prefix given") &&
      dtn_ptr_valid(str, "No string given")) {
...
char const *dtn_string_sanitize(char const *str) {
...
char const *dtn_string_ltrim(char const *str, char const *chars_to_skip) {
...
char *dtn_string_rtrim(char *str, char const *chars_to_skip) {
...
uint16_t dtn_string_to_uint16(const char *string, bool *ok) {
  ...
  *ok = false;
  ...
  if (0 == end_ptr) {
    ...
    dtn_log_error("strtol set end_ptr to 0?");
    exit(1);
  }

  if (string == end_ptr) {

    dtn_log_error("Empty string");
    goto error;
  }

  if (0 != *end_ptr) {

    dtn_log_error("Not a valid integer: %s", string);
    goto error;
  }
  ...
  if (long_number != (long)uint16_number) {
    ...
    dtn_log_error("Number is out of range: %li", long_number);
    goto error;
  }
...
uint32_t dtn_string_to_uint32(const char *string, bool *ok) {
...
bool dtn_string_to_uint64(const char *string, uint64_t size, uint64_t *number) {

  if (!string || size < 1 || !number)
    return false;

  return dtn_convert_string_to_uint64(string, size, number);
}

/*----------------------------------------------------------------------------*/

bool dtn_string_to_int64(const char *string, uint64_t size, int64_t *number) {

  if (!string || size < 1 || !number)
    return false;

  return dtn_convert_string_to_int64(string, size, number);
}

/*----------------------------------------------------------------------------*/

bool dtn_string_to_double(const char *string, uint64_t size, double *number) {

  if (!string || size < 1 || !number)
    return false;

  return dtn_convert_string_to_double(string, size, number);
}

/*****************************************************************************
                            dtn_string_value_for_key
 ****************************************************************************/

static char *strip_quotes(char *s) {

  if (!dtn_ptr_valid(s, "Invalid string")) {
    ...
    if (0 != endquote) {
      *endquote = 0;
      ...
      dtn_log_error("Missing final quotes: %s", s);
      return 0;
    }
...
static dtn_buffer *get_tag_value(char *s, char const *key, char separator) {

  if (dtn_ptr_valid(s, "Invalid token") && dtn_ptr_valid(key, "Invalid key")) {

    s = (char *)dtn_string_ltrim(s, " ");
    ...
    if (0 != value) {
      *value = 0;
      ++value; // *value now '"'
      if (0 == strcmp(s, key)) {
        dtn_buffer *b = dtn_buffer_from_string(strip_quotes(value));
        return b;
      } else {
        return 0;
      }
...
dtn_buffer *dtn_string_value_for_key(char const *s, char const *key,
  ...
  if (dtn_ptr_valid(s, "Invalid token") && dtn_ptr_valid(key, "Invalid key") &&
      dtn_ptr_valid(separator, "No separator to split key-value list given")) {

    dtn_buffer *copy = dtn_buffer_from_string(s);
    char *s = (char *)copy->start;
    char *sp = 0;

    dtn_buffer *value_buf = 0;
...
dtn_data_function dtn_string_data_functions() {

  dtn_data_function functions = {

      .clear = dtn_string_data_clear,
      .free = dtn_string_data_free,
      .copy = dtn_string_data_copy,
      .dump = dtn_string_data_dump};
...
bool dtn_string_data_clear(void *data) {
...
void *dtn_string_data_free(void *data) {
...
void *dtn_string_data_copy(void **destination, const void *string) {

  if (!destination || !string)
    return NULL;

  if (*destination)
    dtn_string_data_free(*destination);
...
bool dtn_string_data_dump(FILE *stream, const void *string) {
...
dtn_list *dtn_string_pointer(const char *source, size_t sc_len, const char *delim,
  ...
  char *pointer = (char *)dtn_string_find(source, sc_len, delim, dm_len);
  ...
  while (pointer) {
    ...
    pointer =
        (char *)dtn_string_find(start, sc_len - (start - source), delim, dm_len);
  }
...
dtn_list *dtn_string_split(const char *source, size_t sc_len, const char *delim,
  ...
  dtn_list *list_pointer = NULL;
  dtn_list *list_copies = NULL;

  list_pointer = dtn_string_pointer(source, sc_len, delim, dm_len);
  if (!list_pointer)
    goto error;

  list_copies = dtn_list_create((dtn_list_config){
      .item = dtn_data_string_data_functions(),
  });
  ...
  for (size_t i = 1; i <= items; i++) {
    ...
    if (i < items) {
      ...
      if (dtn_string_find(ptr1, length, delim, dm_len))
        if (!copy_delimiter)
          length -= dm_len;
    }
...
bool dtn_string_replace_all(char **result, size_t *size, const char *source,
  ...
  if ((0 == result) || (0 == size) || (0 == source) || (0 == sc_len) ||
    ...
    ensure_capacity(result, size, dtn_STRING_DEFAULT_SIZE);
    ...
    for (size_t i = 0; i < true_sc_len; ++i) {
      ...
      if (delim1[delim_index] != source[i]) {
        ...
        str_append(result, &result_capacity, &write_index, delim2, delim2_len);

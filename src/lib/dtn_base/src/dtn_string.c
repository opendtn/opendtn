...
static dtn_buffer *get_tag_value(char *s, char const *key, char separator) {
    ...
    if (dtn_ptr_valid(s, "Invalid token") &&
        dtn_ptr_valid(key, "Invalid key")) {
...
dtn_buffer *dtn_string_value_for_key(char const *s, char const *key,
    ...
    if (dtn_ptr_valid(s, "Invalid token") &&
        dtn_ptr_valid(key, "Invalid key") &&
        dtn_ptr_valid(separator,
                      "No separator to split key-value list given")) {
...
dtn_list *dtn_string_pointer(const char *source, size_t sc_len,
    ...
    while (pointer) {
        ...
        pointer = (char *)dtn_string_find(start, sc_len - (start - source),
                                          delim, dm_len);
    }

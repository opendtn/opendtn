...
bool dtn_dir_tree_remove(const char *path) {
...
static bool mkdir_recursive(char *path) {
    ...
    *last_occurrence = 0;
    ...
    if (!mkdir_recursive(path)) {
        ...
        dtn_log_error("Could not create %s", path);
        goto error;
    }
...
bool dtn_dir_tree_create(char const *path) {
    ...
    if (!dtn_uri_path_remove_dot_segments(path, clean))
        goto error;
...
bool dtn_dir_access_to_path(const char *path) {

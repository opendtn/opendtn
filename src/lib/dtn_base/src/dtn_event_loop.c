...
bool dtn_event_loop_set(dtn_event_loop *self, int sfd, uint8_t events,
  ...
  if (dtn_cond_valid(-1 < sfd, "Cannot set callback on socket fd: Invalid fd") &&
      dtn_ptr_valid(self, "Cannot set callback on event loop: Invalid loop")) {
...
static bool accept_callback(int socket_fd, uint8_t events, void *data) {
  ...
  case AF_INET6:
    ...
    if (!dtn_socket_parse_sockaddr_storage(&local_sa, local_ip, DTN_HOST_NAME_MAX,
                                          &local_port)) {
  ...
  if (local_ip[0] != 0) {
    ...
    dtn_log_debug("accepted at socket fd %i | "
                 ...
                 "new connection fd %i",
                 socket_fd, local_ip, local_port, remote_ip, remote_port, nfd);
...
dtn_event_loop_config
dtn_event_loop_config_from_json(const dtn_item *value) {
  ...
  double timers =
      dtn_item_get_number(dtn_item_object_get(obj, DTN_EVENT_LOOP_KEY_MAX_TIMERS));

...
int test_dtn_event_add_default_connection_accept() {
    ...
    testrun(!dtn_event_add_default_connection_accept(
        NULL, socketTCP, DTN_EVENT_IO_IN, &buffer, cb));
    testrun(!dtn_event_add_default_connection_accept(loop, 0, DTN_EVENT_IO_IN,
                                                     &buffer, cb));
    testrun(!dtn_event_add_default_connection_accept(loop, socketTCP, 0,
                                                     &buffer, cb));
    testrun(!dtn_event_add_default_connection_accept(
        loop, socketTCP, DTN_EVENT_IO_IN, NULL, NULL));

    // TCP OK
    testrun(dtn_event_add_default_connection_accept(
        loop, socketTCP, DTN_EVENT_IO_IN, &buffer, cb));

    // LOCAL OK
    testrun(dtn_event_add_default_connection_accept(
        loop, socketLOCAL, DTN_EVENT_IO_IN, &buffer, cb));

    // UDP NOK
    testrun(!dtn_event_add_default_connection_accept(
        loop, socketUDP, DTN_EVENT_IO_IN, &buffer, cb));
...
int test_dtn_event_loop_config_to_json() {
    ...
    testrun(1 == dtn_item_get_number(
                     dtn_item_object_get(val, DTN_EVENT_LOOP_KEY_MAX_TIMERS)));
    testrun(15 == dtn_item_get_number(dtn_item_object_get(
                      val, DTN_EVENT_LOOP_KEY_MAX_SOCKETS)));
    val = dtn_item_free(val);

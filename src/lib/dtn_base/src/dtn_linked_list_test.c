...
int expand_list_to_position_test() {
    ...
    dtn_list *list = dtn_linked_list_create((dtn_list_config){
...
int test_dtn_linked_list_create() {
...
int check_insert() {
    ...
    testrun(dtn_list_insert(list, 1, eins));
    ...
    void *item = dtn_list_pop(list);
    while (item) {
        free(item);
        item = dtn_list_pop(list);
    }
...
int test_dtn_linked_list_enable_caching() {
    ...
    dtn_linked_list_enable_caching(0);

    /* Required to use dtn_LIST_PERFORM_INTERFACE_TESTS */
    int testrun_counter = 0;
    int result = 0;
    testrun(0 == testrun_counter);

    DTN_LIST_PERFORM_INTERFACE_TESTS(dtn_linked_list_create);

    dtn_registered_cache_free_all();

    /* With small cache */
    dtn_linked_list_enable_caching(3);
...
int test_dtn_linked_list_disable_caching() {
...
int all_tests() {
    ...
    testrun_test(test_dtn_linked_list_create);
    ...
    testrun_test(test_dtn_linked_list_enable_caching);

...
static void dummy_receive_no_drop(void *userdata, int socket, dtn_item *value) {
...
static void dummy_receive_drop(void *userdata, int socket, dtn_item *value) {
...
int test_dtn_json_io_buffer_free() {
    ...
    dtn_memory_pointer ptr = (dtn_memory_pointer){
...
int test_dtn_json_io_buffer_drop() {
    ...
    for (size_t i = 1; i < 10; i++) {
        ...
        testrun(dtn_json_io_buffer_push(
            self, key,
            (dtn_memory_pointer){.start = (uint8_t *)str,
                                 .length = strlen(str)}));

...
*//**
        ...
        @brief          Implementation of dtn_json_grammar.
...
bool dtn_json_validate_string(const uint8_t *buffer, size_t size, bool quotes) {
...
bool dtn_json_match_string(uint8_t **start, uint8_t **end, size_t size) {
    ...
    while (dtn_json_is_whitespace(ptr[0])) {
    ...
    *end = (uint8_t *)(ptr - 1);
    ...
    if (dtn_json_validate_string(*start, ((*end - *start) + 1), false))
        return true;
...
bool dtn_json_is_whitespace(uint8_t byte) {
...
bool dtn_json_clear_whitespace(uint8_t **buffer, size_t *length) {
    ...
    while (dtn_json_is_whitespace((uint8_t)ptr[0])) {
...
bool dtn_json_match_array(uint8_t **start, uint8_t **end, size_t size) {
...
bool dtn_json_match_object(uint8_t **start, uint8_t **end, size_t size) {
...
bool match_string(uint8_t *ptr, size_t size, uint8_t **next) {
    ...
    *next = nxt + 1;
    ...
    if (nxt[0] == 0x22) {
        ...
        if (!dtn_json_validate_string(ptr, (nxt - ptr + 1), true))
            goto error;
    }
...
bool match_object(uint8_t *start, size_t size, struct json_counter *counter,
    ...
    *next = start + 1;
    ...
    while (dtn_json_is_whitespace(*next[0])) {
...
bool dtn_json_match(const uint8_t *start, size_t size, bool incomplete,
    ...
    while (length > 0) {
        ...
        dtn_json_clear_whitespace(&next, (size_t *)&length);
    }

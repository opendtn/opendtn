...
*/
static int dtn_base64_alphabetPosition(uint8_t byte, uint8_t alphabet[]) {
...
static bool dtn_base64_encode_with_alphabet(const uint8_t *source,
...
bool dtn_base64_encode(const uint8_t *buffer, size_t length, uint8_t **result,
                       size_t *result_length) {

    return dtn_base64_encode_with_alphabet(buffer, length, result,
                                           result_length, base64Alphabet);
}
...
bool dtn_base64_url_encode(const uint8_t *buffer, size_t length,
    ...
    return dtn_base64_encode_with_alphabet(buffer, length, result,
                                           result_length, base64urlAlphabet);
}
...
static bool dtn_base64_decode_with_alphabet(const uint8_t *source,
                                            size_t source_length,
                                            uint8_t **dest, size_t *dest_length,
                                            uint8_t alphabet[]) {
    ...
    *dest_length = length;
    ...
    while (k + 4 < source_length) {
        ...
        pos0 = dtn_base64_alphabetPosition(source[k], alphabet);
        if (pos0 < 0)
            goto error;

        pos1 = dtn_base64_alphabetPosition(source[k + 1], alphabet);
        if (pos1 < 0)
            goto error;

        pos2 = dtn_base64_alphabetPosition(source[k + 2], alphabet);
        if (pos2 < 0)
            goto error;

        pos3 = dtn_base64_alphabetPosition(source[k + 3], alphabet);
        if (pos3 < 0)
            goto error;
...
bool dtn_base64_decode(const uint8_t *buffer, size_t length, uint8_t **result,
    ...
    return dtn_base64_decode_with_alphabet(buffer, length, result,
                                           result_length, base64Alphabet);
}

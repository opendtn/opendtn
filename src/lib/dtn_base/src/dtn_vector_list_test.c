...
*//**
        ...
        @brief          Unit tests of dtn_vector_list.
...
int test_dtn_vector_list_set_shrink() {
  ...
  testrun(list);
  testrun(!vector);
  testrun(!dtn_vector_list_set_shrink(NULL, true));
  testrun(!dtn_vector_list_set_shrink(list, true));
  testrun(!dtn_vector_list_set_shrink(list, false));
  list = list->free(list);

  list = dtn_vector_list_create((dtn_list_config){0});
  testrun(list);
...
int test_dtn_vector_list_set_rate() {
  ...
  testrun(list);
  testrun(!vector);
  testrun(!dtn_vector_list_set_rate(NULL, 0));
  testrun(!dtn_vector_list_set_rate(list, 0));
  testrun(!dtn_vector_list_set_rate(list, 100));
  list = list->free(list);
  ...
  testrun(!dtn_vector_list_set_rate(list, 0));
  testrun(vector->config.rate == VECTOR_DEFAULT_SIZE);
  testrun(dtn_vector_list_set_rate(list, 1));
...
int all_tests() {
  ...
  testrun_test(test_dtn_vector_list_set_shrink);
  testrun_test(test_dtn_vector_list_set_rate);

  testrun_test(testing_vector_resize);
  testrun_test(testing_vector_auto_adjust);
  testrun_test(testing_vector_list_is_empty);

  DTN_LIST_PERFORM_INTERFACE_TESTS(dtn_vector_list_create);

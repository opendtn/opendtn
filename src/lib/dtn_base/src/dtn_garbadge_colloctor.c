...
static bool handle_in_thread(dtn_thread_loop *tloop, dtn_thread_message *msg){
    ...
    if (!dtn_thread_lock_try_lock(&self->data.lock)) return true;

    dtn_list *queue = self->data.queue;

    self->data.queue = dtn_linked_list_create((dtn_list_config){
        .item.free = collector_data_free
    });
...
static bool timer_run_cleanup(uint32_t id, void *data){
    ...
    dtn_thread_message *msg = dtn_thread_message_standard_create(
        DTN_GENERIC_MESSAGE, NULL);

    if (!msg) goto reschedule;

    if (!dtn_thread_loop_send_message(self->tloop, msg, 
        DTN_RECEIVER_THREAD)){
        msg = dtn_thread_message_free(msg);
    }
...
reschedule:
    ...
    self->timer.collect = dtn_event_loop_timer_set(
        self->config.loop, 
        self->config.limits.run_cleanup_usec,
        self,
        timer_run_cleanup);
...
bool dtn_garbadge_colloctor_push(
    ...
    if (!self || !data || !free_data) goto error;

    collector = calloc(1, sizeof(CollectorData));
    if (!collector) goto error;

/***
        ...
        ------------------------------------------------------------------------
*//**
        @file           dtn_dict_test_interface.c
...
dtn_dict *(*dict_creator)(dtn_dict_config);
...
int test_impl_dict_is_empty() {
  ...
  dtn_dict *dict = dict_creator(dtn_dict_string_key_config(1));
  testrun(dict);
  ...
  dict = dict_creator(dtn_dict_string_key_config(100));
  testrun(dict);
  testrun(dtn_dict_is_valid(dict));
  testrun(!dict->is_empty(NULL));
  testrun(dict->is_empty(dict));
...
int test_impl_dict_free() {
  ...
  dict = dict_creator(dtn_dict_string_key_config(1));
  testrun(dict);
  key = strdup("key1");
  testrun(dict->set(dict, key, NULL, NULL));
  key = strdup("key2");
...
int test_impl_dict_set() {
  ...
  testrun(dtn_dict_is_set(dict, NULL));
...
int test_impl_dict_del() {
  ...
  dict->config.value.data_function.free = dtn_data_string_free;
...
static bool pair_checker(const void *key, void *value, void *data) {
  ...
  DTN_ASSERT(0 != arg);
  DTN_ASSERT(PAIR_CHECKER_MAGIC_BYTES == arg->magic_bytes);
...
int test_impl_dict_for_each() {
  ...
  testrun(dtn_dict_for_each(dict, &arg, pair_checker));

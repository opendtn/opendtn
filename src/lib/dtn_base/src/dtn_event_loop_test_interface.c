/***
        ...
        ------------------------------------------------------------------------
*//**
        @file           dtn_event_loop_test_interface.c
        @author         Markus Toepfer
        @author         Michael Beer

        @date           2019-04-05

        @ingroup        dtn_event_loop_test_interface
...
static void *run_the_loop(void *arg) {
    ...
    dtn_event_loop *loop = dtn_event_loop_cast(arg);
    if (!loop)
        goto error;
...
static void *run_the_loop_with_timeout(void *arg) {
    ...
    dtn_event_loop *loop = dtn_event_loop_cast(container->loop);
    if (!loop) {
        DTN_ASSERT(1 == 0);
        goto error;
    }
...
static bool counting_timestamped_cb(uint32_t id, void *data) {
    ...
    container->counter++;
    container->timestamp = dtn_time_get_current_time_usecs();
    /*
...
dtn_event_loop *(*event_loop_creator)(dtn_event_loop_config config);
...
int test_impl_event_loop_free() {
    ...
    dtn_event_loop *loop = event_loop_creator(config);
    testrun(loop);
    ...
    loop = event_loop_creator(dtn_event_loop_config_default());
    loop->magic_byte = 0;
    ...
    dtn_socket_configuration socket_config = {.host = "localhost", .type = TCP};

    int socket = dtn_socket_create(socket_config, false, NULL);
    testrun(socket >= 0);
    testrun(dtn_socket_get_config(socket, &socket_config, NULL, NULL));
...
int test_impl_event_loop_callback_set() {
    ...
    for (i = 0; i < 10; i++) {
        ...
        sockets[i] = dtn_socket_create(socket_config, false, NULL);
        if (first == -1)
            first = sockets[i];
    ...
    testrun(!loop->callback.set(NULL, first, DTN_EVENT_IO_IN, NULL,
                                dummy_callback));
    testrun(
        !loop->callback.set(loop, 0, DTN_EVENT_IO_IN, NULL, dummy_callback));
    ...
    testrun(dtn_event_remove_default_connection_accept(loop, last));

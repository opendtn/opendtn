...
static void genrand_reseed() {
  ...
  init_genrand(dtn_time_get_current_time_usecs());
...
void dtn_random_reseed() { genrand_reseed(); }

/*----------------------------------------------------------------------------*/

bool dtn_random_bytes_with_zeros(uint8_t *buffer, size_t size) {
...
bool dtn_random_bytes(uint8_t *buffer, size_t size) {
...
bool dtn_random_string(char **buffer, size_t length, const char *alphabet_in) {

  const char *alphabet = DTN_OR_DEFAULT(
  ...
  for (index = 0; index < length - 1; ++index) {
    pointer[index] = alphabet[dtn_random_range(0, alphabet_max_index)];
  }
...
uint64_t dtn_random_range(uint64_t min, uint64_t max) {
...
bool dtn_random_gaussian(double *r1, double *r2) {

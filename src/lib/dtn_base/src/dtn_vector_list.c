...
*//**
        ...
        @brief          Implementation of dtn_vector_list.
...
#define AS_VECTOR_LIST(x)                                                      \
  (((dtn_list_cast(x) != 0) && (IMPL_VECTOR_LIST_TYPE == ((dtn_list *)x)->type)) \
       ? (VectorList *)(x)                                                     \
       : 0)
...
static bool impl_vector_list_is_empty(const dtn_list *self);
static bool impl_vector_list_clear(dtn_list *self);
static dtn_list *impl_vector_list_free(dtn_list *self);
static size_t impl_vector_list_get_pos(const dtn_list *self, const void *item);
static void *impl_vector_list_get(dtn_list *self, size_t pos);
static bool impl_vector_list_set(dtn_list *self, size_t pos, void *item,
                                 void **old);
static bool impl_vector_list_insert(dtn_list *self, size_t pos, void *item);
static void *impl_vector_list_remove(dtn_list *self, size_t pos);
static bool impl_vector_list_push(dtn_list *self, void *item);
static void *impl_vector_list_pop(dtn_list *self);
static size_t impl_vector_list_count(const dtn_list *self);
static bool impl_vector_list_for_each(dtn_list *self, void *data,
                                      bool (*function)(void *item, void *data));

static void *impl_vector_list_iter(dtn_list *self);
static void *impl_vector_list_next(dtn_list *self, void *iter, void **element);
...
bool dtn_vector_list_set_shrink(dtn_list *self, bool shrink) {
...
bool dtn_vector_list_set_rate(dtn_list *self, size_t rate) {
...
static bool vector_resize(VectorList *vector, size_t slots) {
  ...
  if (slots == 0 || vector->last >= slots - 1)
    goto error;

  if (vector->size == slots - 1)
    return true;
...
static bool impl_vector_list_insert(dtn_list *self, size_t pos, void *item) {
  ...
  if (pos > list->last)
    return self->set(self, pos, item, NULL);

  if (list->last == list->size - 1)
    if (!vector_resize(list, list->size + list->config.rate))
      goto error;
...
static bool impl_vector_list_for_each(dtn_list *self, void *data,
  ...
  if (vector_list_is_empty(list))
    return true;

  for (size_t i = 1; i <= list->last; i++) {

    if (!function(list->items[i], data))
      goto error;
  }
...
void *impl_vector_list_iter(dtn_list *self) {
  ...
  // empty?
  if (impl_vector_list_is_empty(self))
    return NULL;
...
void *impl_vector_list_next(dtn_list *self, void *iter, void **element) {
  ...
  if (iter - *list->items > (int64_t)list->last)
    goto error;

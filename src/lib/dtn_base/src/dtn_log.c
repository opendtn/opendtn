...
#include "../include/dtn_arch.h"
#include "../include/dtn_hashtable.h"
#include "../include/dtn_log_rotate.h"
...
static bool format_as_plain_text(char **msg, size_t *msg_len, int level,
    ...
    if (0 > fprintf(msg_out, " [%s] - %s:%d (%s): ",
                    dtn_log_level_to_string(level), file, line, function)) {
        goto error;
    }
...
#define DEFAULT_LEVEL DTN_LOG_DEBUG
...
static module_output *module_output_create() {
     ...
     */
    mout->func_outputs = dtn_hashtable_create(
        10, (dtn_hashtable_funcs){.key_free = free,
...
bool dtn_log_init() {
    ...
    g_default_output.func_outputs = dtn_hashtable_create(
        100, (dtn_hashtable_funcs){.key_free = free,
                                   .key_copy = string_copy,
                                   .key_cmp = string_compare,
                                   .hash = hash_c_string});
...
static bool close_output_nocheck(dtn_log_output *out) {
...
static dtn_hashtable *free_outputs(dtn_hashtable *table);
...
static dtn_hashtable *free_outputs(dtn_hashtable *table) {
    ...
    dtn_hashtable_for_each(table, free_output, 0);
    return dtn_hashtable_free(table);
}
...
static format_message_func formatter_for_format(dtn_log_format fmt) {
...
static int set_module_output(module_output *mout, const dtn_log_output output,
                             dtn_log_level level) {
    ...
    mout->output.log_rotation.path =
        strdup_if_not_null(output.log_rotation.path);
...
int dtn_log_set_output(char const *module_name, char const *function_name,
    ...
    module_output *mout =
        dtn_hashtable_get(g_default_output.func_outputs, module_name);

    if (0 == mout) {
        mout = module_output_create();
        dtn_hashtable_set(g_default_output.func_outputs, module_name, mout);
    }
    ...
    module_output *fout = dtn_hashtable_get(mout->func_outputs, function_name);

    if (0 == fout) {
        fout = func_output_create();
        dtn_hashtable_set(mout->func_outputs, function_name, fout);
    }
...
static module_output *output_for(char const *module, char const *func) {
    ...
    module_output *mout =
        dtn_hashtable_get(g_default_output.func_outputs, fname);

    if (0 == mout) {
        return &g_default_output;
    }

    module_output *fout = dtn_hashtable_get(mout->func_outputs, func);
...
static bool rotate_log_if_required(module_output *mout) {
    ...
    if (!dtn_log_rotate_files(path, mout->output.log_rotation.max_num_files)) {
        return false;
    }
...
void dtn_log_mute() { g_muted = true; }

/*----------------------------------------------------------------------------*/

void dtn_log_unmute() { g_muted = false; }

...
static int check_concurrent_access() {
  ...
  dtn_registered_cache *cache = dtn_registered_cache_extend("multithreaded", cfg);
...
static dtn_registered_cache_sizes *sizes_from_string(char const *str) {
  ...
  DTN_ASSERT(0 != jval);
  dtn_registered_cache_sizes *sizes = dtn_registered_cache_sizes_from_json(jval);
...
#define CACHE_NAME_2 "gullinborsti"
  ...
  jval = json_from_string("{\"" "enabled" "\": false}");
  testrun(0 != jval);
...
static bool check_caches_enabled(dtn_hashtable *sizes, char const **cache_names,
  ...
  for (size_t i = 0; i < num_names; ++i) {
    ...
    dtn_registered_cache *cache = dtn_hashtable_get(g_registry, cache_names[i]);
...
static int test_dtn_registered_cache_sizes_configure() {
  ...
  dtn_registered_cache_sizes *sizes =
      sizes_from_string("{\"enabled\":true}");

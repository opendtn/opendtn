/***
        ...
        ------------------------------------------------------------------------
*//**
        @file           dtn_event_loop_poll.c
        @author         Markus Toepfer
        @author         Michael Beer

        @date           2018-12-17

        @ingroup        dtn_event_loop_poll
...
#define AS_POLL_LOOP(x)                                                        \
  (((dtn_event_loop_cast(x) != 0) &&                                            \
    (IMPL_POLL_LOOP_TYPE == ((dtn_event_loop *)x)->type))                       \
       ? (PollLoop *)(x)                                                       \
       : 0)

static bool flags_dtn_to_poll(uint8_t *dtn_flag, short *poll_event);
...
static bool poll_loop_init(PollLoop *loop, dtn_event_loop_config config);
static dtn_event_loop *impl_poll_event_loop_free(dtn_event_loop *self);

static bool impl_poll_event_loop_is_running(const dtn_event_loop *self);
static bool impl_poll_event_loop_stop(dtn_event_loop *self);
static bool impl_poll_event_loop_run(dtn_event_loop *self, uint64_t max_runtime);

static bool impl_poll_event_loop_callback_set(
    dtn_event_loop *self, int socket_fd, uint8_t events, void *data,
    bool (*callback)(int socket_fd, uint8_t events, void *data));

static bool impl_poll_event_loop_callback_unset(dtn_event_loop *self,
                                                int socket_fd, void **userdata);

static uint32_t
impl_poll_event_loop_timer_set(dtn_event_loop *self, uint64_t relative_usec,
                               void *data,
                               bool (*callback)(uint32_t id, void *data));

static bool impl_poll_event_loop_timer_unset(dtn_event_loop *self, uint32_t id,
                                             void **userdata);

/*---------------------------------------------------------------------------*/

bool flags_dtn_to_poll(uint8_t *dtn_flag, short *poll_event) {

  if (!dtn_flag || !poll_event)
    goto error;
...
bool poll_loop_init(PollLoop *loop, dtn_event_loop_config config) {
  ...
  if (config.max.sockets < IMPL_SOCKETS_MIN) {
    dtn_log_error("Eventloop config with less then minimum sockets "
                 "%i",
                 IMPL_SOCKETS_MIN);
    goto error;
  }
  ...
  if (!loop->fds) {
    dtn_log_error("Failed to allocate bytes"
                 " for poll fds.");
    goto error;
  }
  ...
  dtn_log_debug("Eventloop allocated  %" PRIu64 " bytes", size);
  ...
  if (loop->wakeup[0] >= (int)loop->config.max.sockets) {
    dtn_log_error("Cannot create eventloop, socket index to small "
...
dtn_event_loop *dtn_event_loop_poll(dtn_event_loop_config config) {
  ...
  if ((config.max.sockets < 1) || (config.max.timers < 1)) {
    dtn_log_error("Eventloop config not supported.");
    goto error;
  }
...
dtn_event_loop *impl_poll_event_loop_free(dtn_event_loop *self) {
  ...
  if (loop->fds)
    loop->fds = dtn_data_pointer_free(loop->fds);

  if (loop->cb.socket)
    loop->cb.socket = dtn_data_pointer_free(loop->cb.socket);

  if (loop->cb.timer)
    loop->cb.timer = dtn_data_pointer_free(loop->cb.timer);
...
bool impl_poll_event_loop_is_running(const dtn_event_loop *self) {
...
bool impl_poll_event_loop_stop(dtn_event_loop *self) {
...
static int calculate_timeout_msec(PollLoop *loop, uint64_t start_usec,
  ...
  // dtn_log_debug("next_usec %zu", next_usec);
...
bool impl_poll_event_loop_run(dtn_event_loop *self, uint64_t max) {
  ...
  do {
    ...
    if (result > 0) {
      ...
      dtn_log_error("Could not run poll errno %i|%s", errno, strerror(errno));
      goto error;
    }
...
bool impl_poll_event_loop_callback_set(
  ...
  short poll_event = 0;
  if (!flags_dtn_to_poll(&events, &poll_event))
    goto error;
  ...
  if ((uint32_t)socket >= loop->config.max.sockets) {
    dtn_log_error("EVENTLOOP out of socket slots");
    goto error;
  }
  ...
  DTN_ASSERT(loop->fds[socket].fd == socket);
...
bool impl_poll_event_loop_callback_unset(dtn_event_loop *self, int socket,
                                         void **userdata) {
...
uint32_t impl_poll_event_loop_timer_set(dtn_event_loop *self,
...
bool impl_poll_event_loop_timer_unset(dtn_event_loop *self, uint32_t id,
                                      void **userdata) {

...
*//**
        ...
        ------------------------------------------------------------------------
*/
#include "../include/dtn_mc_test_common.h"
...
static bool interface_permit_ipv4(const struct sockaddr_in *sock) {
  ...
  char host[DTN_HOST_NAME_MAX + 1] = {0};

  if (!inet_ntop(AF_INET, &sock->sin_addr, host, DTN_HOST_NAME_MAX))
    goto error;
  /*
...
static bool interface_permit_ipv6(const struct sockaddr_in6 *sock) {
  ...
  if (!inet_ntop(AF_INET6, &sock->sin6_addr, host, DTN_HOST_NAME_MAX))
    goto error;
...
int dtn_mc_test_common_open_socket(int net) {
  ...
  for (ifa = ifaddr, n = 0; ifa != NULL; ifa = ifa->ifa_next, n++) {
    ...
    socket = -1;
    socket_config = (dtn_socket_configuration){0};
    ...
    case AF_INET:
      ...
      if (interface_permit_ipv4((struct sockaddr_in *)ifa->ifa_addr)) {
        ...
        if (0 == getnameinfo(ifa->ifa_addr, sizeof(struct sockaddr_in),
                             socket_config.host, DTN_HOST_NAME_MAX, NULL, 0,
                             NI_NUMERICHOST)) {
    ...
    socket = dtn_socket_create(socket_config, false, NULL);

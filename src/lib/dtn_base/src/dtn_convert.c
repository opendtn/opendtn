...
bool dtn_convert_string_to_uint64(const char *string, uint64_t size,
  ...
  return dtn_convert_string_to_uint(string, size, number, UINT64_MAX);
}
...
bool dtn_convert_string_to_uint32(const char *string, uint64_t size,
  ...
  if (!dtn_convert_string_to_uint(string, size, &nbr, UINT32_MAX))
    return false;
...
bool dtn_convert_string_to_uint16(const char *string, uint64_t size,
  ...
  if (!dtn_convert_string_to_uint(string, size, &nbr, UINT16_MAX))
    return false;
...
bool dtn_convert_double_to_string(double number, char **string, size_t *size) {
...
bool dtn_convert_int64_to_string(int64_t number, char **string, size_t *size) {
...
bool dtn_convert_uint64_to_string(uint64_t number, char **string, size_t *size) {
...
bool dtn_convert_uint32_to_string(uint32_t number, char **string, size_t *size) {
...
bool dtn_convert_hex_to_binary(const uint8_t *hex, size_t hex_length,
  ...
  for (size_t i = 0; i < count; i += 2) {
    ...
    if (!hextobin((char)clean[i], &one) || !hextobin((char)clean[i + 1], &two))
      goto error;
    ptr[k] = (one << 4) | two;
    k++;
  }
...
double dtn_convert_to_db(double multiplier) {

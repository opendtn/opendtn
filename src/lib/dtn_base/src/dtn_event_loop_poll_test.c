/***
        ...
        ------------------------------------------------------------------------
*//**
        @file           dtn_event_loop_poll_test.c
...
int test_dtn_event_loop_poll() {

    dtn_event_loop_config config = {

        .max.sockets = 10, .max.timers = 10};

    dtn_event_loop *eventloop = dtn_event_loop_poll(config);
    testrun(eventloop);
...
int check_flags_dtn_to_poll() {

    uint8_t dtn_flag = 0;
    short poll_flag = 0;

    testrun(!flags_dtn_to_poll(NULL, NULL));
    testrun(!flags_dtn_to_poll(&dtn_flag, NULL));
    testrun(!flags_dtn_to_poll(NULL, &poll_flag));
    ...
    dtn_flag = DTN_EVENT_IO_IN | DTN_EVENT_IO_OUT | DTN_EVENT_IO_CLOSE |
               DTN_EVENT_IO_ERR;
...
bool dummy_timer_callback(uint32_t id, void *data) {
    ...
    uint64_t current = dtn_time_get_current_time_usecs();
...
int check_poll_loop() {
    ...
    dtn_event_loop *loop = dtn_event_loop_poll(config);
    testrun(loop);
...
int all_tests() {
    ...
    testrun_test(test_dtn_event_loop_poll);
    testrun_test(check_flags_dtn_to_poll);
    testrun_test(check_poll_loop);

    testrun_test(check_interval_call);
    testrun_test(check_calculate_timeout_msec);

    DTN_EVENT_LOOP_PERFORM_INTERFACE_TESTS(dtn_event_loop_poll);

...
dtn_io_buffer *dtn_io_buffer_create(dtn_io_buffer_config config){
    ...
    dtn_dict_config d_config = (dtn_dict_config){
        .slots = 255,
...
bool dtn_io_buffer_push(dtn_io_buffer *self, 
    ...
    if (!self || !remote || !buffer || size < 1) goto error;

    val = dtn_buffer_create(size);
    if (!dtn_buffer_push(val, (uint8_t*) buffer, size)) goto error;
    ...
    if (!dtn_dict_set(self->dict, key, val, NULL)) goto error;
    return true;
error:

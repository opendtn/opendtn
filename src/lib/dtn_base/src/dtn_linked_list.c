...
static dtn_registered_cache *g_list_cache = 0;
static dtn_registered_cache *g_list_entity_cache = 0;
...
/*
 For each function *must* hold:
        ...
        DTN_ASSERT((0 == list->head.next) || (0 == list->head.next->last));
        DTN_ASSERT((0 == list->head.last) || (0 == list->head.last->next));

On exit!

The list is either empty:

        DTN_ASSERT((0 == list->head.next) && (0 == list->head.last));
...
#define AS_LINKED_LIST(x)                                                      \
  (((dtn_list_cast(x) != 0) && (LINKED_LIST_TYPE == ((dtn_list *)x)->type))      \
       ? (LinkedList *)(x)                                                     \
       : 0)
...
static void release_list_entity(ListEntity *entity) {
  ...
  entity = dtn_registered_cache_put(g_list_entity_cache, entity);
...
static ListEntity *acquire_list_entity() {
  ...
  ListEntity *entity = dtn_registered_cache_get(g_list_entity_cache);
...
void release_list(LinkedList *list) {
  ...
  list = dtn_registered_cache_put(g_list_cache, list);

  if (0 != list) {

    list = dtn_list_free(&list->public);
  }
...
static ListEntity *get_entity_at_position(const dtn_list *list, size_t pos);
/**
 * Runs to list and returns entity at pos - 1.
 * Expands list if list is shorter than pos - 1.
 */
static ListEntity *expand_list_to_position(dtn_list *list, size_t pos);

static bool impl_linked_list_is_empty(const dtn_list *self);
static bool impl_linked_list_clear(dtn_list *self);
static dtn_list *impl_linked_list_free(dtn_list *self);
static size_t impl_linked_list_get_pos(const dtn_list *self, const void *item);
static void *impl_linked_list_get(dtn_list *self, size_t pos);
static bool impl_linked_list_set(dtn_list *self, size_t pos, void *item,
                                 void **old);
static bool impl_linked_list_insert(dtn_list *self, size_t pos, void *item);
static void *impl_linked_list_remove(dtn_list *self, size_t pos);
static bool impl_linked_list_push(dtn_list *self, void *item);
static void *impl_linked_list_pop(dtn_list *self);
static size_t impl_linked_list_count(const dtn_list *self);
static bool impl_linked_list_for_each(dtn_list *self, void *data,
                                      bool (*function)(void *item, void *data));

static void *impl_linked_list_iter(dtn_list *self);
static void *impl_linked_list_next(dtn_list *self, void *iter, void **element);
...
dtn_list *dtn_linked_list_create(dtn_list_config config) {
  ...
  dtn_list_default_implementations default_implementations =
      dtn_list_get_default_implementations();

  LinkedList *list = dtn_registered_cache_get(g_list_cache);
  ...
  *list = (LinkedList){
  ...
  return dtn_list_set_magic_bytes((dtn_list *)list);
}
...
static inline ListEntity *get_entity_at_position(const dtn_list *list,
                                                 size_t pos) {
...
static inline ListEntity *expand_list_to_position(dtn_list *self, size_t pos) {
...
static dtn_list *impl_linked_list_free(dtn_list *self) {
  ...
  self = dtn_registered_cache_put(g_list_cache, self);
...
void *impl_linked_list_iter(dtn_list *self) {
...
void *impl_linked_list_next(dtn_list *self, void *iter, void **element) {
...
static void *linked_list_free(void *vptr) {
  ...
  dtn_list *ll = dtn_list_cast(vptr);
...
void dtn_linked_list_enable_caching(size_t capacity) {
  ...
  g_list_cache = dtn_registered_cache_extend("linked_list", cfg);

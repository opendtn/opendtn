...
static dtn_registered_cache *cache_free(dtn_registered_cache *restrict self) {
  ...
  dtn_log_debug(
      ...
      self->stats.put_called, self->stats.elements_put, self->stats.get_called,
      self->stats.elements_got, self->next_free, no_elements_freed);
...
bool dtn_registered_cache_sizes_configure(dtn_registered_cache_sizes *cfg) {
  ...
  ok =
      ok & enable_caching_for(cfg, dtn_linked_list_enable_caching, "list");
...
dtn_item *dtn_registered_cache_report(dtn_json *json,
                                          dtn_item *target) {
...
static bool add_cache_size(const char *vkey, const dtn_item *value, void *varg) {
  ...
  if (!dtn_item_is_number(value)) {
    ...
    dtn_log_error("Malformed config: Expected number for key %s, got %s", key,
                 cval == 0 ? "0" : cval);
    free(cval);

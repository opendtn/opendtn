...
static bool configure_log_rotation(dtn_log_output *out,
                                   dtn_item const *jval) {

  dtn_item const *max_num_messages_jval =
      dtn_item_get(jval, "/messages_per_file");

  dtn_item const *max_num_files_jval =
      dtn_item_get(jval, "/num_files");
  ...
  if ((0 != max_num_files_jval) && (!dtn_item_is_number(max_num_files_jval))) {
...
bool configure_function(const void *key, void *value, void *module_name) {
  ...
  dtn_log_info("Log: Configuring for module %s function %s", module_name, key);

...
*//**
        ...
        @brief          Implementation of dtn_time
...
bool dtn_timestamp_write_to(bool micro, char *buffer, size_t size) {
...
char *dtn_time_string(dtn_time_scope_t scope) {
...
dtn_time dtn_timestamp_create() {

    char *timestamp = dtn_timestamp(false);
    dtn_time out = dtn_timestamp_from_string(timestamp);
    free(timestamp);
    return out;
}
...
bool dtn_time_write_to(dtn_time_scope_t scope, char *buffer, size_t size) {
...
uint64_t dtn_time_get_abs_timeout_usec(uint64_t rel_timeout_usec) {
...
uint64_t dtn_time_get_current_time_usecs() {
...
static bool time_to_string_usec_nocheck(char *target, size_t size,
    ...
    DTN_ASSERT(USECS_PER_SEC * secs <= timeval);
...
bool dtn_time_to_string(char *target, size_t size, uint64_t timeval,
                        dtn_time_unit unit) {
...
dtn_time dtn_timestamp_from_string(const char *timestamp) {
    ...
    dtn_timestamp_parse(timestamp, &out.year, &out.month, &out.day, &out.hour,
...
char *dtn_timestamp_to_string(dtn_time time) {

    return dtn_timestamp_from_data(time.year, time.month, time.day, time.hour,
                                   time.minute, time.second);
}

/*----------------------------------------------------------------------------*/

uint64_t dtn_time_to_epoch(dtn_time time) {

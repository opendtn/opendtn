...
int test_dtn_random_bytes() {

  size_t size = 1000000;
  uint8_t buffer[size];
  memset(buffer, 0, size);

  testrun(!dtn_random_bytes(NULL, 0));

  size_t sz = 1;
  testrun(dtn_random_bytes(buffer, sz));
  ...
  testrun(dtn_random_bytes(buffer, size));
  ...
  // check randomness
  uint8_t *array[1000];
  srandom(dtn_time_get_current_time_usecs());
  size_t bytes = 100;
  ...
  for (size_t i = 0; i < 1000; i++) {
    ...
    testrun(dtn_random_bytes(array[i], bytes));
...
int test_dtn_random_bytes_with_zeros() {
...
int test_dtn_random_string() {
  ...
  for (i = 0; i < testValues; i++) {
    ...
    testrun(dtn_random_string(&buffer1, bufferLength, 0));
  ...
  const char test_alphabet[] = "12aCe3wertzuiop";

  for (i = 0; i < testValues; i++) {

    buffer1 = calloc(bufferLength, sizeof(uint8_t));

    testrun(dtn_random_string(&buffer1, bufferLength, test_alphabet));
...
int test_dtn_random_uint32() {
  ...
  for (size_t i = 0; i < 100000; ++i) {
    check_in_or_out_of_circle(dtn_random_uint32(), dtn_random_uint32(),
                              UINT32_MAX);
  }
...
int test_dtn_random_uint64() {
  ...
  for (size_t i = 0; i < 100000; ++i) {
    check_in_or_out_of_circle(dtn_random_uint64(), dtn_random_uint64(),
                              UINT64_MAX);
  }
...
int test_dtn_random_range() {

  uint64_t out = dtn_random_range(0, 0);
  testrun(out > 0);
  out = dtn_random_range(1000, 0);
  testrun(out >= 1000);
  out = dtn_random_range(0, 1000);
  testrun(out <= 1000);
  out = dtn_random_range(500, 1000);
  testrun(out <= 1000);
  testrun(out >= 500);

  out = dtn_random_range(500, 500);
  testrun(out <= 500);
  testrun(out >= 500);

  out = dtn_random_range(500, 501);
  testrun(out <= 501);
  testrun(out >= 500);

  out = dtn_random_range(100000, 100001);
  testrun(out <= 100001);
  testrun(out >= 100000);

  out = dtn_random_range(100, 100001);
  testrun(out <= 100001);
  testrun(out >= 100);

  for (uint64_t i = 1; i < 10000; i++) {

    out = dtn_random_range(0, i);
    testrun(out <= i);
    out = dtn_random_range(i, 0);
    testrun(out >= i);
    out = dtn_random_range(i / 2, i);
    testrun(out <= i);
    testrun(out >= i / 2);
  }

  for (size_t i = 0; i < 100000; ++i) {
    check_in_or_out_of_circle(dtn_random_range(14, 829),
                              dtn_random_range(14, 829), 829 - 14);
  }
...
static int test_dtn_random_gaussian() {

  testrun(!dtn_random_gaussian(0, 0));

  double r1 = 0.0;

  testrun(dtn_random_gaussian(&r1, 0));

  fprintf(stderr, "Gaussian: %f\n", r1);

  double r2 = 0;

  testrun(dtn_random_gaussian(&r1, &r2));
  testrun(dtn_random_gaussian(0, &r2));
...
int all_tests() {
  ...
  testrun_init();
  testrun_test(test_dtn_random_bytes_with_zeros);
  testrun_test(test_dtn_random_bytes);
  testrun_test(test_dtn_random_string);
  testrun_test(test_dtn_random_uint32);
  testrun_test(test_dtn_random_uint64);
  testrun_test(test_dtn_random_range);
  testrun_test(test_dtn_random_gaussian);

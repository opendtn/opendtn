...
bool dtn_socket_ensure_nonblocking(int socket) {
    ...
    if (0 != getsockopt(socket, SOL_SOCKET, SO_TYPE, &opt, &opt_len)) {
        dtn_log_error("Could not get type of socket %i errno %i | %s", socket,
...
dtn_socket_data dtn_socket_data_from_json(const dtn_item *value) {
    ...
    const char *host =
        dtn_item_get_string(dtn_item_object_get(value, DTN_KEY_SOCKET_HOST));

    if (host)
        strncpy(data.host, host, DTN_HOST_NAME_MAX);

    data.port = (uint16_t)dtn_item_get_number(
        dtn_item_object_get(value, DTN_KEY_SOCKET_PORT));
...
bool dtn_socket_log(int socket, const char *message, ...) {
    ...
    if (dtn_socket_get_sockaddr_storage(socket, &local_sa, &remote_sa, NULL)) {
        ...
        if (local_ip[0] != 0) {
            ...
            dtn_log_debug("socket fd %i | "
                          "PATH %s | %s",
                          socket, ((struct sockaddr_un *)&local_sa)->sun_path,
                          msg);
        }
...
bool dtn_socket_configuration_to_sockaddr(const dtn_socket_configuration in,
    ...
    if (0 == sockaddr_len) {
        dtn_log_error(
            "Expected pointer to sockaddr_storage_len, got 0 pointer");
        goto error;
    }

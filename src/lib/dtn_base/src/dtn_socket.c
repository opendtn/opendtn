...
*//**
        ...
        @brief          Implementation of a dtn_base socket interface.
...
#define DTN_KEY_SOCKET_TYPE "type"
#define DTN_KEY_SOCKET_HOST "host"
#define DTN_KEY_SOCKET_PORT "port"

#define DTN_SOCKET_TYPE_STRING_TCP "TCP"
#define DTN_SOCKET_TYPE_STRING_UDP "UDP"
#define DTN_SOCKET_TYPE_STRING_TLS "TLS"
#define DTN_SOCKET_TYPE_STRING_DTLS "DTLS"
#define DTN_SOCKET_TYPE_STRING_LOCAL "LOCAL"

#define DTN_SOCKET_MSG_MAX 2500
...
int dtn_socket_close_local(int socket) {
  if (socket < 0)
    goto error;
...
int socket_create_local(dtn_socket_configuration config, bool as_client,
                        dtn_socket_error *err_return) {
  if (config.host[0] == 0)
    return -1;
...
static int setup_server_socket_nocheck(int fd,
  ...
  dtn_socket_set_reuseaddress(fd);
...
int dtn_socket_create(dtn_socket_configuration config, bool as_client,
  ...
  for (struct addrinfo *current = res; 0 != current;
    ...
    // dtn_socket_dump_addrinfo(stdout, current);
...
int dtn_socket_create_at_interface(struct ifaddrs *interface,
  ...
  e.gai = getnameinfo(interface->ifa_addr,
                      ...
                      config.host, DTN_HOST_NAME_MAX, NULL, 0, NI_NUMERICHOST);

  if (e.gai != 0)
    goto error;

  sfd = dtn_socket_create(config, false, &e);
  if (sfd < 0)
    goto error;
...
int dtn_socket_get_send_buffer_size(int socket) {
  int size = 0;
...
int dtn_socket_get_recv_buffer_size(int socket) {
  int size = 0;
  socklen_t len = sizeof(size);
...
bool dtn_socket_set_dont_fragment(int socket) {
  if (socket < 0)
    goto error;
...
bool dtn_socket_ensure_nonblocking(int socket) {
  int opt = 0;
  socklen_t opt_len = sizeof(opt);

  if (0 != getsockopt(socket, SOL_SOCKET, SO_TYPE, &opt, &opt_len)) {
    dtn_log_error("Could not get type of socket %i errno %i | %s", socket, errno,
                 strerror(errno));
    goto error;
  }
...
bool dtn_socket_set_reuseaddress(int socket) {
  int opt = 1;
...
bool dtn_socket_disable_nagl(int fh) {
  if (0 > fh) {
    return false;
...
bool dtn_socket_disable_delayed_ack(int fh) {
  if (0 > fh) {
    return false;
...
bool dtn_socket_config_from_sockaddr_storage(const struct sockaddr_storage *sa,
  ...
  case AF_INET:
    ...
    if (0 ==
        inet_ntop(AF_INET, &sock4->sin_addr, config->host, DTN_HOST_NAME_MAX)) {
      e.err = errno;
      goto error;
    }
  ...
  case AF_UNIX:
    ...
    port = 0;
    strncpy(config->host, so_un->sun_path, DTN_HOST_NAME_MAX);
    break;
...
bool dtn_socket_get_config(int fd, dtn_socket_configuration *local,
  ...
  dtn_socket_transport transport = NETWORK_TRANSPORT_TYPE_ERROR;
  ...
  if (0 != local) {
    ...
    if (!dtn_socket_config_from_sockaddr_storage(&sa, local, &e))
      goto error;
  }
  ...
  if (0 != remote) {
    ...
    if (!dtn_socket_config_from_sockaddr_storage(&sa, remote, &e))
      goto error;
...
bool dtn_socket_parse_sockaddr_storage(const struct sockaddr_storage *sa_master,
...
dtn_socket_data
dtn_socket_data_from_sockaddr_storage(const struct sockaddr_storage *sa) {
  dtn_socket_data data;
  memset(&data, 0, sizeof(dtn_socket_data));
  ...
  if (!dtn_socket_parse_sockaddr_storage(sa, data.host, DTN_HOST_NAME_MAX,
                                        &data.port))
    goto error;
...
bool dtn_socket_get_data(int socket, dtn_socket_data *local,
  ...
  if (!dtn_socket_get_sockaddr_storage(socket, &sa_local, &sa_remote, NULL))
    goto error;

  if (local)
    *local = dtn_socket_data_from_sockaddr_storage(&sa_local);

  if (remote)
    *remote = dtn_socket_data_from_sockaddr_storage(&sa_remote);
...
#define dtn_SOCKET_TYPE_STRING_LOWER_TCP "tcp"
#define dtn_SOCKET_TYPE_STRING_LOWER_UDP "udp"
#define dtn_SOCKET_TYPE_STRING_LOWER_TLS "tls"
#define dtn_SOCKET_TYPE_STRING_LOWER_DTLS "dtls"
#define dtn_SOCKET_TYPE_STRING_LOWER_LOCAL "local"

/*---------------------------------------------------------------------------*/

const char *dtn_socket_transport_to_string(dtn_socket_transport type) {
  switch (type) {
  case TCP:
    return dtn_SOCKET_TYPE_STRING_TCP;

  case UDP:
    return dtn_SOCKET_TYPE_STRING_UDP;

  case TLS:
    return dtn_SOCKET_TYPE_STRING_TLS;

  case DTLS:
    return dtn_SOCKET_TYPE_STRING_DTLS;

  case LOCAL:
    return dtn_SOCKET_TYPE_STRING_LOCAL;
...
const char *dtn_socket_transport_to_string_lower(dtn_socket_transport type) {
  switch (type) {
  case TCP:
    return dtn_SOCKET_TYPE_STRING_LOWER_TCP;

  case UDP:
    return dtn_SOCKET_TYPE_STRING_LOWER_UDP;

  case TLS:
    return dtn_SOCKET_TYPE_STRING_LOWER_TLS;

  case DTLS:
    return dtn_SOCKET_TYPE_STRING_LOWER_DTLS;

  case LOCAL:
    return dtn_SOCKET_TYPE_STRING_LOWER_LOCAL;
...
dtn_socket_transport dtn_socket_transport_from_string(const char *string) {
  ...
  for (dtn_socket_transport i = NETWORK_TRANSPORT_TYPE_ERROR + 1;
       i < NETWORK_TRANSPORT_TYPE_OOB; i++) {
    if (0 == strncasecmp(dtn_socket_transport_to_string(i), string, len))
      return i;
  }
...
dtn_socket_transport dtn_socket_transport_parse_string(const char *string,
  ...
  if (0 == strncasecmp(dtn_SOCKET_TYPE_STRING_TCP, string, length))
    if (length == strlen(dtn_SOCKET_TYPE_STRING_TCP))
      return TCP;

  if (0 == strncasecmp(dtn_SOCKET_TYPE_STRING_UDP, string, length))
    if (length == strlen(dtn_SOCKET_TYPE_STRING_UDP))
      return UDP;

  if (0 == strncasecmp(dtn_SOCKET_TYPE_STRING_TLS, string, length))
    if (length == strlen(dtn_SOCKET_TYPE_STRING_TLS))
      return TLS;

  if (0 == strncasecmp(dtn_SOCKET_TYPE_STRING_DTLS, string, length))
    if (length == strlen(dtn_SOCKET_TYPE_STRING_DTLS))
      return DTLS;

  if (0 == strncasecmp(dtn_SOCKET_TYPE_STRING_LOCAL, string, length))
    if (length == strlen(dtn_SOCKET_TYPE_STRING_LOCAL))
      return LOCAL;
...
dtn_socket_configuration dtn_socket_configuration_from_item(
  ...
  if (!dtn_item_is_object(object))
    goto error;

  dtn_item *value =
      dtn_item_object_get((dtn_item *)object, DTN_KEY_SOCKET_TYPE);

  if (value) {
    dtn_socket_transport transport =
        dtn_socket_transport_from_string(dtn_item_get_string(value));
  ...
  value = dtn_item_object_get((dtn_item *)object, DTN_KEY_SOCKET_HOST);

  if (value) {
    const char *content = dtn_item_get_string(value);
    if (!content)
      goto error;

    if (!strncpy(config.host, content, DTN_HOST_NAME_MAX))
      goto error;
  }

  value = dtn_item_object_get((dtn_item *)object, DTN_KEY_SOCKET_PORT);

  if (value) {
    double port = dtn_item_get_number(value);
  ...
  return config;
error:
  return (dtn_socket_configuration){0};
}

/*----------------------------------------------------------------------------*/

bool dtn_socket_configuration_to_item(const dtn_socket_configuration config,
                                     dtn_item **destination) {
  bool created = false;
  if (!destination)
    goto error;

  if (!*destination) {
    *destination = dtn_item_object();
    if (!*destination)
      goto error;
  ...
  dtn_item *value = NULL;
  dtn_item *object = *destination;

  if (config.host[0]) {
    value = dtn_item_string(config.host);
    if (0 == value)
      goto error;
  ...
  if (!dtn_item_object_set(object, DTN_KEY_SOCKET_HOST, value))
    goto error;

  value = dtn_item_number(config.port);

  if (0 == value)
    goto error;

  if (!dtn_item_object_set(object, DTN_KEY_SOCKET_PORT, value))
    goto error;

  const char *type = dtn_socket_transport_to_string(config.type);
  if (!type)
    goto error;

  value = dtn_item_string(type);
  if (0 == value)
    goto error;

  if (!dtn_item_object_set(object, DTN_KEY_SOCKET_TYPE, value))
    goto error;

  return true;
error:
  if (created)
    *destination = dtn_item_free(*destination);
  return false;
}
...
bool dtn_socket_configuration_equals(dtn_socket_configuration cfg1,
  ...
  if (0 != strncmp(cfg1.host, cfg2.host, DTN_HOST_NAME_MAX))
    return false;
...
dtn_item *dtn_socket_data_to_json(const dtn_socket_data *data) {
  dtn_item *out = NULL;
  dtn_item *val = NULL;
  ...
  if (0 == data->host[0]) {
    ...
    val = dtn_item_string(data->host);
  }
  if (!dtn_item_object_set(out, DTN_KEY_SOCKET_HOST, val))
    goto error;

  val = dtn_item_number(data->port);
  if (!dtn_item_object_set(out, DTN_KEY_SOCKET_PORT, val))
    goto error;
...
dtn_socket_data dtn_socket_data_from_json(const dtn_item *value) {
  dtn_socket_data data;
  memset(&data, 0, sizeof(dtn_socket_data));
  if (!value)
    goto error;

  const char *host = dtn_item_get_string(dtn_item_object_get(value, DTN_KEY_SOCKET_HOST));

  if (host)
    strncpy(data.host, host, DTN_HOST_NAME_MAX);

  data.port =
      (uint16_t)dtn_item_get_number(dtn_item_object_get(value, DTN_KEY_SOCKET_PORT));
...
bool dtn_socket_log_error_with_config(dtn_socket_configuration config,
  ...
  return dtn_log_error("Failed to open socket\n"
                      "HOST:PORT|TYPE %s:%i|%s\n"
                      "errno          %i|%s\n"
                      "gaierror       %i|%s\n",
                      config.host, config.port,
                      dtn_socket_transport_to_string(config.type), err.err,
                      strerror(err.err), err.gai, gai_strerror(err.gai));
}

/*---------------------------------------------------------------------------*/

bool dtn_socket_log(int socket, const char *message, ...) {
  if (socket < 0)
    return false;
  ...
  char msg[DTN_SOCKET_MSG_MAX] = {0};

  if (message) {
    va_list args;
    va_start(args, message);
    vsnprintf(msg, DTN_SOCKET_MSG_MAX, message, args);
    va_end(args);
  }

  // get local SA
  if (dtn_socket_get_sockaddr_storage(socket, &local_sa, &remote_sa, NULL)) {
    // parse debug logging data
    switch (local_sa.ss_family) {
    case AF_INET:
    case AF_INET6:

      if (!dtn_socket_parse_sockaddr_storage(&local_sa, local_ip,
                                            DTN_HOST_NAME_MAX, &local_port)) {
      ...
      if (!dtn_socket_parse_sockaddr_storage(&remote_sa, remote_ip,
                                            DTN_HOST_NAME_MAX, &remote_port)) {
        dtn_log_error("Failed to parse data "
                     "from socket fd %i",
                     socket);
        return false;
    ...
    dtn_log_debug("socket fd %i | %s", socket, msg);
  }
...
static bool socket_configuration_clear(void *data) {
  ...
  memset(data, 0, sizeof(dtn_socket_configuration));
  return true;
}
...
static void *socket_configuration_copy(void **dest, const void *src) {
  ...
  if (!*dest) {
    *dest = calloc(1, sizeof(dtn_socket_configuration));
    if (!*dest)
      goto error;
  ...
  if (!memcpy(*dest, src, sizeof(dtn_socket_configuration)))
    goto error;
...
static bool socket_configuration_dump(FILE *stream, const void *data) {
  ...
  dtn_socket_configuration *config = (dtn_socket_configuration *)data;
...
dtn_list *dtn_socket_configuration_list() {
  dtn_list *list = dtn_list_create((dtn_list_config){
...
bool dtn_socket_generate_5tuple(char **dest, size_t *dest_len, int socket,
  ...
  memset(&local, 0, sizeof(dtn_socket_configuration));

  if (!dtn_socket_get_config(socket, &local, NULL, NULL))
    goto error;

  const char *transport = dtn_socket_transport_to_string(local.type);
  if (!transport)
    goto error;
...
bool dtn_socket_is_dgram(int socket) {
  if (socket < 0)
    goto error;
...
int dtn_socket_get_type(int socket) {
  if (socket < 0)
    goto error;
...
bool dtn_sockets_are_similar(int socket1, int socket2) {
  ...
  if (!dtn_socket_get_sockaddr_storage(socket1, &sa1, NULL, NULL))
    goto error;

  if (!dtn_socket_get_sockaddr_storage(socket2, &sa2, NULL, NULL))
    goto error;

  if (sa1.ss_family != sa2.ss_family)
    goto error;

  int eins = dtn_socket_get_type(socket1);
  int zwei = dtn_socket_get_type(socket2);
...
bool dtn_socket_connect(int fd, const struct sockaddr_storage *remote) {
  if (fd < 1 || !remote)
    goto error;

  dtn_socket_data local = {0};

  if (!dtn_socket_get_data(fd, &local, NULL))
    goto error;
...
bool dtn_socket_unconnect(int fd) {
  if (fd < 0)
    goto error;
...
error:
  if (fd != 0)
    dtn_log_critical("Could not unconnect fd %i", fd);
...
bool dtn_socket_destination_address_type(const char *host, int *type) {
...
int dtn_socket_open_server(dtn_socket_configuration *scfg) {
  int sfd = -1;
  dtn_socket_data us = {0};

  if (!dtn_ptr_valid(scfg,
                    "Cannot open server socket: No socket configuration") ||
      (!dtn_cond_valid(-1 < (sfd = dtn_socket_create(*scfg, false, 0)),
                      "Opening server socket failed: IO error")) ||
      (!dtn_cond_valid(dtn_socket_get_data(sfd, &us, 0),
                      "Could not parse back server socket information")) ||
      (!dtn_cond_valid(sizeof(scfg->host) > strlen(us.host),
                      "Host name too long to be kept in socket "
                      "configuration"))) {
    dtn_socket_close(sfd);
    return -1;
...
dtn_socket_load_dynamic_port(dtn_socket_configuration config) {
  dtn_socket_configuration actual = config;
  int sfd = dtn_socket_open_server(&actual);
  dtn_socket_close(sfd);
...
uint32_t dtn_socket_get_max_supported_runtime_sockets(uint32_t sockets) {
  ...
  if (limit.rlim_cur > UINT32_MAX) {
    ...
    TODO("... fix rlim_t > UINT32_MAX in dtn_*");
    DTN_ASSERT(1 == 0);
    return 0;
  }
...
static bool get_sockaddr(int domain, dtn_socket_transport transport,
  ...
  default:
    ...
    dtn_log_error("Unsupported transport type");
    goto error;
  };
...
bool dtn_socket_configuration_to_sockaddr(const dtn_socket_configuration in,
                                         ...
                                         socklen_t *sockaddr_len) {
  if (0 == sockaddr) {
    dtn_log_error("Expected sockaddr_storage, got 0 pointer");
    goto error;
  }

  if (0 == sockaddr_len) {
    dtn_log_error("Expected pointer to sockaddr_storage_len, got 0 pointer");
    goto error;
  }
...
dtn_socket_data
dtn_socket_configuration_to_socket_data(const dtn_socket_configuration in) {
  dtn_socket_data out = {0};

  socklen_t len = sizeof(out.sa);

  if (!dtn_socket_configuration_to_sockaddr(in, &out.sa, &len))
    goto error;

  if (!dtn_socket_parse_sockaddr_storage(&out.sa, out.host, DTN_HOST_NAME_MAX,
                                        &out.port))
    goto error;

  return out;
error:
  return (dtn_socket_data){0};
}

/*----------------------------------------------------------------------------*/

bool dtn_socket_state_from_handle(int socket_handle, dtn_socket_state *state) {
  dtn_socket_data local = {0};
  dtn_socket_data remote = {0};

  if (!dtn_ptr_valid(state, "Could get socket state: No target state object")) {
    return false;

  } else if (!dtn_socket_get_data(socket_handle, &local, &remote)) {
    dtn_log_error("Could not get state for socket handle %i", socket_handle);
    return false;

  } else {
    *state = (dtn_socket_state){

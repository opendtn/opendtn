...
#define AS_DEFAULT_DICT(x)                                                     \
  (((dtn_dict_cast(x) != 0) &&                                                  \
    (IMPL_DEFAULT_DICT_TYPE == ((dtn_dict *)x)->type))                          \
       ? (DefaultDict *)(x)                                                    \
       : 0)
...
static bool impl_dict_is_empty(const dtn_dict *self);
static bool impl_dict_clear(dtn_dict *self);
static dtn_dict *impl_dict_free(dtn_dict *self);

static dtn_list *impl_dict_get_keys(const dtn_dict *self, const void *val);
static void *impl_dict_get(const dtn_dict *self, const void *key);
static bool impl_dict_set(dtn_dict *self, void *key, void *value,
                          void **replaced);
static bool impl_dict_del(dtn_dict *self, const void *key);
static void *impl_dict_remove(dtn_dict *self, const void *key);

static bool impl_dict_for_each(dtn_dict *self, void *data,
                               bool (*function)(const void *key, void *value,
                                                void *data));

/*---------------------------------------------------------------------------*/

static bool dict_calculate_slot(const dtn_dict *self, const void *key,
                                size_t *slot) {

  if (!self || !slot)
    return false;

  if (!dtn_dict_config_is_valid(&self->config))
    return false;
...
static dict_pair *get_start_pair_at_slot(const dtn_dict *self, size_t slot) {
...
dtn_dict *dtn_dict_cast(const void *data) {
  ...
  if (*(uint16_t *)data != dtn_DICT_MAGIC_BYTE)
    return NULL;

  return (dtn_dict *)data;
}
...
bool dtn_dict_config_is_valid(const dtn_dict_config *config) {
  ...
  if (!config || (config->slots < 1) || !config->key.hash || !config->key.match)
    return false;
...
bool dtn_dict_is_valid(const dtn_dict *dict) {
  ...
  if (!dtn_dict_cast(dict))
    return false;

  if (!dtn_dict_config_is_valid(&dict->config))
    return false;
...
dtn_dict *dtn_dict_create(dtn_dict_config config) {
  ...
  if (!dtn_dict_config_is_valid(&config))
    goto error;
  ...
  if (!dtn_dict_set_magic_bytes(dict))
    goto error;
...
bool dtn_dict_is_set(const dtn_dict *self, const void *key) {
...
int64_t dtn_dict_count(const dtn_dict *self) {
  ...
  if (!self->for_each((dtn_dict *)self, &counter, count_keys))
    goto error;
...
static bool impl_dict_is_empty(const dtn_dict *self) {
  ...
  if (!self->for_each((dtn_dict *)self, NULL, key_is_empty))
    goto error;
...
static bool check_value_at_key(const void *key, void *value, void *data) {
  ...
  if (!dtn_list_cast(container->list))
    return false;
...
static dtn_list *impl_dict_get_keys(const dtn_dict *self, const void *val) {
  ...
  keys = dtn_list_create((dtn_list_config){0});
  ...
  if (!self->for_each((dtn_dict *)self, &container, collector))
    goto error;
...
static bool dict_pair_replace_value(dtn_dict *self, dict_pair *pair, void *value,
                                    void **replaced) {
...
dtn_dict_config dtn_dict_string_key_config(size_t slots) {
  ...
  return (dtn_dict_config){
      ...
      .slots = slots,
      .key.data_function = dtn_data_string_data_functions(),
...
dtn_dict_config dtn_dict_uint64_key_config(size_t slots) {
  ...
  return (dtn_dict_config){
      ...
      .slots = slots,
      .key.data_function = dtn_data_uint64_data_functions(),
...
void *dtn_dict_copy(void **destination, const void *data) {
  ...
  copy = dtn_dict_cast(*destination);
  if (!copy)
    goto error;
...
bool dtn_dict_dump(FILE *stream, const void *data) {
  ...
  size_t count = dtn_dict_count(dict);
...
static dtn_list *dict_load_factor(const dtn_dict *dict) {
  ...
  for (size_t i = 0; i < dict->config.slots; i++) {
    ...
    dtn_list_push(list, (void *)counter);
  }
...
bool dtn_dict_dump_load_factor(FILE *stream, const dtn_dict *dict) {
  ...
  dtn_list *list = dict_load_factor(dict);
  if (!list)
    goto error;

  uint64_t counter = dtn_list_count(list);
  fprintf(stream, "DICT with %" PRIu64 " items LOAD ---> \n", counter);

  uint64_t items_count = 0;
  for (uint64_t i = 0; i < counter; i++) {

    intptr_t items = (intptr_t)dtn_list_get(list, i + 1);
    items_count += items;
    if (0 == items)
      continue;

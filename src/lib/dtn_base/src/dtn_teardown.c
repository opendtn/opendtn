...
DestructorEntry *get_destructors() {
    ...
    g_destructors = DTN_OR_DEFAULT(g_destructors,
                                   destructor_entry(0, "teardown system", 0));
...
bool dtn_teardown_register_multiple(void (*destructor)(void),
                                    char const *name) {
...
static DestructorEntry *destruct(DestructorEntry *destructor) {
    ...
    if (0 != destructor) {
        if (0 != destructor->destructor) {
            dtn_log_info("Destructing %s",
                         dtn_string_sanitize(destructor->name));

...
DestructorEntry *destructor_entry(void (*destructor)(void), char const *name,
  ...
  entry->name = dtn_string_dup(DTN_OR_DEFAULT(name, "UNKNOWN"));
  entry->next = next;
  return entry;
}
...
DestructorEntry *get_destructors() {
  ...
  g_destructors =
      DTN_OR_DEFAULT(g_destructors, destructor_entry(0, "teardown system", 0));
...
bool dtn_teardown_register(void (*destructor)(void), char const *name) {
  ...
  return ((0 != destructor) && (0 != find(get_destructors(), destructor))) ||
         dtn_teardown_register_multiple(destructor, name);
}
...
static DestructorEntry *destruct(DestructorEntry *destructor) {
  ...
  if (0 != destructor) {
    if (0 != destructor->destructor) {
      dtn_log_info("Destructing %s", dtn_string_sanitize(destructor->name));
      destructor->destructor();
    }
    DestructorEntry *entry = destructor->next;
    dtn_free(destructor->name);
    dtn_free(destructor);
    return entry;
  } else {
    return destructor;

...
*//**
        ...
        ------------------------------------------------------------------------
*/
#include "../include/dtn_mc_socket.h"
...
int dtn_mc_socket(dtn_socket_configuration config) {
  ...
  socket = dtn_socket_create(config, false, NULL);
  if (-1 == socket)
    goto error;

  if (!dtn_socket_ensure_nonblocking(socket))
    goto error;
  if (!dtn_socket_set_reuseaddress(socket))
    goto error;
...
bool dtn_mc_socket_drop_membership(int socket) {
  ...
  if (!dtn_socket_get_data(socket, &data, NULL))
    goto error;

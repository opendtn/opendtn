...
typedef struct {
  ...
  dtn_thread_pool_statistics statistics;
...
static bool thread_start(dtn_thread_pool *self);
static bool thread_stop(dtn_thread_pool *self);
static dtn_thread_pool *thread_free(dtn_thread_pool *self);

static void *thread_run(void *arg);

static int try_lock_until_stop(dtn_thread_lock *restrict lock,
                               _Atomic int *state,
...
dtn_thread_pool *dtn_thread_pool_create(dtn_thread_queue incoming,
                                      dtn_thread_pool_function process_func,
                                      dtn_thread_pool_config config) {

  if (!incoming.queue) {

    dtn_log_error("No incoming queue given (0 pointer)");
    goto error;
  }

  if (!process_func) {

    dtn_log_error("Require process function");
    goto error;
  }

  if (0 == config.num_threads) {
    config.num_threads = DTN_DEFAULT_NUM_THREADS;
  }
  ...
  for (size_t i = 0; i < config.num_threads; ++i) {
    ...
    pool->threads[i].pool = (dtn_thread_pool *)pool;
  }

  atomic_init(&pool->pool_state, STOPPED);
  pool->config = config;

  return (dtn_thread_pool *)pool;
...
static bool spawn_threads(internal_pool *restrict pool) {
  ...
  for (size_t i = 0; i < pool->config.num_threads; ++i) {
    ...
    int status =
        pthread_create(&pool->threads[i].id, 0, thread_run, &pool->threads[i]);

    if (0 != status) {

      dtn_log_error("Thread could not be created/started");
      goto error;
    }
  };
  ...
  dtn_log_info("Spawned %zu threads", pool->config.num_threads);
...
static bool thread_start(dtn_thread_pool *self) {
  ...
  case RUNNING:
    ...
    dtn_log_error("Thread already running - not going to "
                 "start");
    goto error;
  ...
  case STOPPED:
    ...
    if (!spawn_threads(pool)) {
      dtn_log_error("Could not spawn threads");
      goto error;
    }
...
static bool thread_stop(dtn_thread_pool *self) {
  ...
  for (size_t i = 0; i < internal->config.num_threads; ++i) {
    ...
    if ((0 != status) || (STOPPED != internal->threads[i].state)) {
      ...
      dtn_log_error("Could not stop thread: %s", strerror(status));
...
static dtn_thread_pool *thread_free(dtn_thread_pool *self) {
  ...
  if ((RUNNING == atomic_load(&internal->pool_state)) && (!self->stop(self))) {

    dtn_log_error("Could not free thread pool - "
                 " still running");
    goto error;
  }

  if (STOPPED != atomic_load(&internal->pool_state)) {
    dtn_log_error("Could not stop thread pool");
    goto error;
  }
...
static void *thread_run(void *arg) {
  ...
  if (!thread) {
    ...
    dtn_log_error("Did not receive a proper thread object (0 "
                 "pointer)");
    goto error;
  }
  ...
  if (!pool) {
    ...
    dtn_log_error("Did not receive a proper pool object (0 "
                 "pointer)");
    goto error;
  }

  dtn_ringbuffer *in = pool->incoming.queue;

  if (!in) {

    dtn_log_error("No incoming queue");
    goto error;
  }
  ...
  if (!process) {
    ...
    dtn_log_error("No processing function set(0 pointer)");
    goto error;
  }

  dtn_thread_lock *in_lock = pool->incoming.lock;
...
static int try_lock_until_stop(dtn_thread_lock *restrict lock,
  ...
  if (lock) {
    ...
    while (!dtn_thread_lock_try_lock(lock)) {

      dtn_log_warning("Could not lock down on incoming queue");

...
------------------------------------------------------------------------
*//**
        @file           dtn_list_test_interface.c
        @author         Michael Beer
...
dtn_list *(*list_creator)(dtn_list_config);
...
int test_impl_list_is_empty() {
  ...
  dtn_list *list = list_creator((dtn_list_config){
...
int test_impl_list_clear() {
  ...
  testrun(list->config.item.clear == dtn_data_string_clear);
  testrun(list->config.item.copy == dtn_data_string_copy);
  testrun(list->config.item.dump == dtn_data_string_dump);
  ...
  // NOT a list
  list->magic_byte = dtn_LIST_MAGIC_BYTE | 0x1234;
  testrun(!list->clear(list));
  list->magic_byte = dtn_LIST_MAGIC_BYTE;
...
int test_impl_list_free() {
  ...
  list = list_creator((dtn_list_config){
      .item = dtn_data_string_data_functions(),
  });
  ...
  list = list_creator((dtn_list_config){0});
  list->push(list, "non allocated content");
  list->push(list, "non allocated content");
  list->push(list, "non allocated content");
  testrun(3 == list->count(list));
...
int test_impl_list_copy() {
  ...
  copy = list_creator((dtn_list_config){
      .item = dtn_data_string_data_functions(),
  });

  dtn_list *copy_2 = list->copy(list, copy);
  testrun(0 != copy);
  testrun(copy == copy_2);
...
int test_impl_list_remove() {
  ...
  dtn_list *list = list_creator((dtn_list_config){0});
...
int test_impl_list_for_each() {
  ...
  testrun(list->for_each(list, &cta, check_teststrings) && (0 == cta.counter));

  testrun(true == list->set(list, 1, teststring, NULL));

  cta.counter = 0;
  testrun(list->for_each(list, &cta, check_teststrings) && (1 == cta.counter));

  testrun(true == list->set(list, 2, teststring2, NULL));
  cta.counter = 0;
  testrun(list->for_each(list, &cta, check_teststrings) && (2 == cta.counter));

  testrun(true == list->set(list, 3, teststring3, NULL));
  cta.counter = 0;
  testrun(list->for_each(list, &cta, check_teststrings) && (3 == cta.counter));
...
int test_impl_list_iter() {
  ...
  testrun(dtn_list_push(list, teststring1));
  ...
  testrun(dtn_list_push(list, teststring2));
  testrun(dtn_list_push(list, teststring3));

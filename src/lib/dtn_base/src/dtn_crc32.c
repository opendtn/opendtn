...
bool dtn_crc32_generate_table_for(uint32_t polynomial, bool reverted_in,
    ...
    if (dtn_ptr_valid(lookup,
                      "Cannot generate CRC32 table: No target memory")) {
...
uint32_t dtn_crc32_posix(uint8_t const *data, size_t len_octets) {

    // Parameters are the same as for OGG, just the final checksum has to be
    // xored with 0xffffffff

    return 0xffffffff ^ dtn_crc32_ogg(0, data, len_octets);
}

/*----------------------------------------------------------------------------*/

uint32_t dtn_crc32_ogg(uint32_t init, uint8_t const *data, size_t len_octets) {

    static bool initialized = false;
    static uint32_t lookup[0x100] = {0};

    if (!initialized) {
        dtn_crc32_generate_table_for(0x04c11db7, false, lookup);
        initialized = true;
    }

    return dtn_crc32_sum(init, data, len_octets, false, false, lookup);
}
...
uint32_t dtn_crc32_zlib(uint32_t init, uint8_t const *data, size_t len_octets) {
    ...
    if (!initialized) {
        dtn_crc32_generate_table_for(0x04c11db7, true, table);
        initialized = true;
    }

    return 0xffffffff ^ dtn_crc32_sum(0xffffffff ^ init, data, len_octets, true,
                                      true, table);
}
...
uint32_t dtn_crc32c(uint8_t const *data, size_t len_octets) {
    ...
    return 0xffffffff ^
           dtn_crc32_sum(0xFFFFFFFF, data, len_octets, true, true, lookup);
}

...
#include "../include/dtn_byteorder.h"
...
const char *dtn_file_read_check(const char *restrict path) {
    ...
    if ((mode == S_IFIFO) || (mode == S_IFREG) || (mode == S_IFSOCK)) {
        ...
        if (0 > fd) {
            return DTN_FILE_COULD_NOT_READ;
        }
...
dtn_file_handle_state dtn_file_read(const char *path, uint8_t **buffer,
    ...
    dtn_file_handle_state state = DTN_FILE_ERROR;
...
dtn_file_handle_state dtn_file_write(const char *path, const uint8_t *buffer,
    ...
    if (fp != NULL) {
        ...
        if (count == size) {
            ...
            dtn_log_debug("WRITE, file (%s), "
                          "wrote %jd bytes.",
                          path, count);

            state = DTN_FILE_SUCCESS;

        } else {

            dtn_log_error("WRITE, file (%s), "
...
bool dtn_file_get_16(uint16_t *out, uint8_t **rd_ptr, size_t *length,
    ...
    *length -= 2;
    ...
    case DTN_FILE_SWAP_BYTES:
        read_16 = DTN_SWAP_16(read_16);
        break;

    case DTN_FILE_LITTLE_ENDIAN:

        read_16 = DTN_LE16TOH(read_16);
        break;

    case DTN_FILE_BIG_ENDIAN:

        read_16 = DTN_BE16TOH(read_16);
        break;
...
bool dtn_file_get_32(uint32_t *out, uint8_t **rd_ptr, size_t *length,
    ...
    *length -= 4;
    ...
    case DTN_FILE_SWAP_BYTES:
        read_32 = DTN_SWAP_32(read_32);
        break;

    case DTN_FILE_LITTLE_ENDIAN:

        read_32 = DTN_LE32TOH(read_32);
        break;

    case DTN_FILE_BIG_ENDIAN:

        read_32 = DTN_BE32TOH(read_32);
        break;

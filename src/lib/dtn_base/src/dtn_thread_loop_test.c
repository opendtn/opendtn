...
static bool configs_equal(dtn_thread_loop_config c1, dtn_thread_loop_config c2) {
...
static void *feeder_thread(void *arg) {
  ...
  for (size_t i = 0; num_messages_to_send > i; ++i) {
    ...
    dtn_thread_loop_send_message(tpp, create_num_message(i), DTN_RECEIVER_THREAD);
  }

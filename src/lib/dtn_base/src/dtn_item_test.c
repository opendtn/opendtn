...
static bool clear_subitem(const char *key, dtn_item const *val,
                          void *userdata) {
...
int check_pointer_replace_special_encoding() {
    ...
    testrun(
        !pointer_replace_special_encoding(NULL, size, buffer1, "abc", "xyz"));

    testrun(
        !pointer_replace_special_encoding(result, size, NULL, "abc", "xyz"));
    ...
    testrun(
        !pointer_replace_special_encoding(result, 0, buffer1, "abc", "xyz"));
    ...
    memset(buffer2, '\0', size);
    testrun(
        pointer_replace_special_encoding(result, size, buffer1, "abc", "x"));
    ...
    memset(buffer2, '\0', size);
    testrun(
        pointer_replace_special_encoding(result, size, buffer1, "vvv", "x"));
    ...
    // replace all
    memset(buffer2, '\0', size);
    testrun(pointer_replace_special_encoding(result, size, buffer1,
                                             "123abcdefg", "x"));
    ...
    memset(buffer2, '\0', size);
    testrun(
        !pointer_replace_special_encoding(result, 5, buffer1, "abc", "xyz"));
    testrun(strlen(buffer2) == 0);

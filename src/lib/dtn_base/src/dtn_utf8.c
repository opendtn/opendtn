...
*//**
        ...
        needed for dtn_service instances.
...
bool dtn_utf8_validate_sequence(const uint8_t *start, uint64_t length) {

  if (!start || length < 1)
    return false;

  uint8_t *result = dtn_utf8_last_valid(start, length, false);
  if (!result)
    return false;
...
uint64_t dtn_utf8_decode_code_point(const uint8_t *p, uint64_t open,
                                   uint64_t *bytes) {
  ...
  *bytes = 0;
  ...
  if (p[0] <= 0x7f) {
    ...
    *bytes = 2;
     ...
     */
    *bytes = 3;
    return (((p[0] << 12) & 0xF000) | ((p[1] << 6) & 0x0FC0) | (p[2] & 0x3F));
...
bool dtn_utf8_generate_random_buffer(uint8_t **buffer, size_t *size,
  ...
  for (uint16_t i = 0; i < unicode_chars; i++) {
    ...
    while (!done) {
      ...
      if (dtn_utf8_encode_code_point(number, &pointer, open, &used))
        done = true;
    }

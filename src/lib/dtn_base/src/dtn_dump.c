...
bool dtn_dump_binary_as_hex(FILE *stream, uint8_t *binary, uint64_t length) {
...
bool dtn_dump_socket_addrinfo(FILE *stream, struct addrinfo *info) {
  ...
  if (info->ai_addr)
    return dtn_dump_socket_sockaddr(stream, info->ai_addr);
...
bool dtn_dump_socket_sockaddr(FILE *stream, struct sockaddr *addr) {
  ...
  if (addr->sa_family == AF_INET) {
    ...
    return dtn_dump_socket_sockaddr_in(stream, (struct sockaddr_in *)addr);

  } else if (addr->sa_family == AF_INET6) {

    return dtn_dump_socket_sockaddr_in6(stream, (struct sockaddr_in6 *)addr);
  }
...
bool dtn_dump_socket_sockaddr_in(FILE *stream, struct sockaddr_in *addr) {
...
bool dtn_dump_socket_sockaddr_in6(FILE *stream, struct sockaddr_in6 *addr) {

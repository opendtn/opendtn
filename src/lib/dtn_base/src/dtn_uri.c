...
#include "../include/dtn_memory_pointer.h"
...
static bool mask_port(const char *start, size_t length, char **item,
                      size_t *len, char **next, dtn_uri *uri) {
...
bool dtn_uri_clear(dtn_uri *uri) {

  if (!uri)
    return false;

  uri->scheme = dtn_data_pointer_free(uri->scheme);
  uri->path = dtn_data_pointer_free(uri->path);
  uri->user = dtn_data_pointer_free(uri->user);
  uri->host = dtn_data_pointer_free(uri->host);
  uri->query = dtn_data_pointer_free(uri->query);
  uri->fragment = dtn_data_pointer_free(uri->fragment);
  uri->scheme = dtn_data_pointer_free(uri->scheme);
  uri->port = 0;
...
bool dtn_uri_string_is_valid(const char *string, size_t length) {
  ...
  if (dtn_uri_string_is_absolute(string, length))
    return true;

  return dtn_uri_string_is_reference(string, length);
}
...
bool dtn_uri_string_is_absolute(const char *start, size_t length) {
...
bool dtn_uri_string_is_reference(const char *start, size_t length) {
...
char *dtn_uri_to_string(const dtn_uri *uri) {
  ...
  size_t len = 0;
  size_t length = dtn_uri_string_length(uri);
  if (0 == length)
    goto error;

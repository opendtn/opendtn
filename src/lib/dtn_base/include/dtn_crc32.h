 ...
 * dtn_crc32_generate_table_for(0x04c11db7, false, ogg_table);
 *
 * uint8_t chunk1[] = {...};
 * uint8_t chunk2[] = {...};
 *
 * uint32_t crc = dtn_crc32_sum(0, chunk1, sizeof(chunk1), false, false,
 * ogg_table);
 *
 * You can do a rolling update of your CRC:
 *
 * crc = dtn_crc32_sum(crc, chunk2, sizeof(chunk2), false, false, ogg_table);
 *
 *
 * Some checksum flavors require you to xor some value onto your checksum
 * after calculation:
 *
 * uint32_t crc32_posix = dtn_crc32_sum(
 ...
 * If you want do to a rolling posix crc calculation, use `dtn_crc32_sum`
 * and finalize with XOR:
 *
 * uint32_t posix_table = {0};
 * dtn_crc32_generate_table_for(0x04c11db7, false, posix_table);
 *
 * uint32_t crc32_posix = dtn_crc32_sum(

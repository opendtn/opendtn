 ...
 * the first dtn_STRING_DEFAULT_SIZE chars...
 * Ensures that the returned duplicate is zero terminated.
 * @return a duplicate of `to_dup`.
 */
char *dtn_string_dup(char const *to_copy);

/*----------------------------------------------------------------------------*/

/**
 * Compares 2 strings in a safe way.
 * Max length of either of the strings is dtn_string_DEFAULT_SIZE
...
/**
 * Like dtn_string_equal, but case insensitively
 */
...
*/
bool dtn_string_data_clear(void *string);
...
*/
void *dtn_string_data_free(void *string);

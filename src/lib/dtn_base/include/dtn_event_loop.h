...
*//**
        ...
        @brief          Definition of a standard eventloop interface
                        ...
                        interface tests for dtn_ eventloops.

                        NOTE for implementors of new loops:
                        Always call `dtn_event_loop_setup_signals()` in your
                        constructor!
...
#define DTN_EVENT_LOOP_MAGIC_BYTE 0xabba

#define DTN_EVENT_LOOP_KEY "eventloop"
#define DTN_EVENT_LOOP_KEY_MAX_SOCKETS "sockets"
#define DTN_EVENT_LOOP_KEY_MAX_TIMERS "timers"

typedef struct dtn_event_loop dtn_event_loop;
typedef struct dtn_event_loop_config dtn_event_loop_config;
typedef struct dtn_event_loop_event dtn_event_loop_event;

/*---------------------------------------------------------------------------*/

struct dtn_event_loop_config {
...
struct dtn_event_loop {
    ...
    dtn_event_loop *(*free)(dtn_event_loop *self);
    bool (*is_running)(const dtn_event_loop *self);

    /*
     *      Stop self from running.
     */
    bool (*stop)(dtn_event_loop *self);
     ...
     *
     *      @NOTE   a max_runtime of dtn_RUN_ONCE
     ...
     */
    bool (*run)(dtn_event_loop *self, uint64_t max_runtime_usec);
    ...
    struct {
         ...
         *      @param events   events to listen to (OR of dtn_events)
         ...
         *
         */
        bool (*set)(dtn_event_loop *self, int socket, uint8_t events,
                    void *userdata,
         ...
         */
        bool (*unset)(dtn_event_loop *self, int socket, void **userdata);
    ...
    struct {
        ...
        uint32_t (*set)(dtn_event_loop *self, uint64_t relative_usec,
                        void *data, bool (*callback)(uint32_t id, void *data));
         ...
         *                      handover with set
         */
        bool (*unset)(dtn_event_loop *self, uint32_t id, void **userdata);
...
/**
        Create a default event loop config
                ...
                {
                        .sockets = dtn_EVENT_LOOP_MAX_SOCKETS_DEFAULT,
                        .timers  = dtn_EVENT_LOOP_MAX_TIMERS_DEFAULT
                } max;
*/
...
bool dtn_event_loop_set(dtn_event_loop *self, int socket, uint8_t events,
                        void *userdata,
                        bool (*callback)(int socket_fd, uint8_t events,
                                         void *userdata));
...
/**
        ...
        dtn_event_loop.
*/
dtn_event_loop *dtn_event_loop_cast(const void *);
...
/**
        ...
        (ANY) dtn_event_loop implementation and add the IO callback to all
        accepted connection fds.

        @NOTE To cleanup data @see dtn_event_remove_default_connection_accept
...
/**
        ...
        Counterpart to @see dtn_event_add_default_connection_accept.
*/
bool dtn_event_remove_default_connection_accept(dtn_event_loop *self,
                                                int socket);
...
/**
...

        {
                "dtn_EVENT_LOOP_KEY" :
                {
                        "dtn_EVENT_LOOP_KEY_MAX_SOCKETS" : 1,
                        "dtn_EVENT_LOOP_KEY_MAX_TIMERS" : 1,
                }
        }
...
/**
        ...
        bigger config using dtn_EVENT_LOOP_KEY
*/

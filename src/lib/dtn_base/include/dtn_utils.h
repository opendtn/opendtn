...
#define DTN_OR_DEFAULT(x, y) ((0 == (x)) ? (y) : (x))
 ...
 *     ...
 *     our_object *o = dtn_UTILS_AS_TYPE(vptr, OUR_OBJECT_MAGIC_BYTES);
 *     ...
 * }
 */
 ...
 */
#define DTN_TRACE fprintf(stderr, "TRACE %s: %i\n", __FUNCTION__, __LINE__)
...
#define DTN_ASSERT(cond) DTN_ASSERT_INTERNAL(cond)
 ...
 *
 */
double dtn_utils_db_to_factor(double db);
...
bool dtn_utils_init_random_generator();
...
/**
 * Search an array of pointer for pointer_to_remdtne and set it to 0 if found.

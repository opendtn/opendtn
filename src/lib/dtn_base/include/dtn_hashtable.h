...
*//**
        ...
        Prdtnides a hash table of arbitrary, variable size.

        Basically, you can
         * dtn_hashtable_set(table, key, value)
         * dtn_hashtable_get(table, key)
         * dtn_hashtable_remdtne(table, key)
         * iterate dtner all entries using dtn_hashtable_for_each(table, func)

        E.g. to find out how many entries a table contains, just use
        dtn_hashtable_for_each() like so:
         ...
         size_t number_of_entries = dtn_hashtable_for_each(table, dummy_proc, 0);
        ...
        to dtn_hashtable_create() however.
        ...
        Example to use this hash table with c strings:
             ...
             // There is a wrapper for this: dtn_hashtable_create_c_string
             dtn_hashtable str_table = dtn_hashtable_create(
             ...
             *four_pointer = 5;
             ...
             dtn_hashtable_set(str_table, "four", (void*) 4);
             assert(4 == (int) dtn_hashtable_get(str_table, "four"));
             // because strcmp(four, "four) == 0 this works fine
             assert(4 == (int) dtn_hashtable_get(str_table, four));

             // Value of four is copied into hash table
             dtn_hashtable_set(str_table, four, four_pointer);
             free(four);
             // four_pointer is NOT copied - the pointer in the hash table
             // is the same as four_pointer
             assert(four_pointer == (int) dtn_hashtable_get(str_table, "four"));

             str_hash = dtn_hashtable_free(str_hash);
             // Internally, the strings have been copied and freed
             // automatically...
             free(five);

        If values need to be freed, you might use
        dtn_hashtable_for_each() and free everything you iterate dtner
        before performing an or dtn_hashtable_free() like so in the example
        abdtne:
             ...
             dtn_hashtable_for_each(str_table, free_value, 0);
        ...
        a hash table, dtn_hashtable_get() will still return its pointer on request.
        ...
        AFTER you remdtned them from the hash or nesure that the hash table is not
        used further after starting to free values contained within.


        ------------------------------------------------------------------------
*/
#ifndef dtn_hashtable_h
#define dtn_hashtable_h
...
typedef struct dtn_hashtable_struct dtn_hashtable;
...
/**
        ...
        You are encouraged to prdtnide dtn_hahshtable_funcs appropriate for the
...
/**
        Remdtnes an entry from hashtable.
 ...
 */
void *dtn_hashtable_remdtne(dtn_hashtable *table, const void *key);

/*---------------------------------------------------------------------------*/

/**
        Iterates dtner all entries in hash and calls process() on them.
...
/**
        Remdtnes all entries from hashtable.
        Does not free the values.
        If you need to free values beforehand,
        use dtn_hashtable_for_each() to free them.
 */

...
dtn_item *dtn_password_hash(const char *password,
    ...
    if ((0 != params.blocksize) || (0 != params.parallel)) {
        ...
        if (!dtn_password_hash_scrypt(hash, &length, password, (char *)salt,
                                      params))
            goto error;
...
dtn_password dtn_password_generate(const char *secret,
    ...
    const char *salt =
        dtn_item_get_string(dtn_item_object_get(item, DTN_AUTH_KEY_SALT));
    ...
    const char *hash =
        dtn_item_get_string(dtn_item_object_get(item, DTN_AUTH_KEY_HASH));
    ...
    out.workfactor =
        dtn_item_get_number(dtn_item_object_get(item, DTN_AUTH_KEY_WORKFACTOR));

    out.blocksize =
        dtn_item_get_number(dtn_item_object_get(item, DTN_AUTH_KEY_BLOCKSIZE));

    out.parallel =
        dtn_item_get_number(dtn_item_object_get(item, DTN_AUTH_KEY_PARALLEL));
...
bool dtn_password_check(const char *secret, dtn_password password) {
    ...
    /* parse the params to be used */
    dtn_password_hash_parameter params =
        (dtn_password_hash_parameter){.workfactor = password.workfactor,

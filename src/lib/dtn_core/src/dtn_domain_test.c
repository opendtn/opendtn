...
int test_dtn_domain_config_to_item() {
  ...
  testrun(out);
  testrun(dtn_item_get(out, "/" "path"));
  testrun(dtn_item_get(out, "/" "name"));
  testrun(dtn_item_get(out, "/" "certificate"));
  testrun(dtn_item_get(out, "/" "certificate" "/" "key"));
  testrun(dtn_item_get(out, "/" "certificate" "/" "file"));
  testrun(dtn_item_get(out, "/" "certificate" "/" "ca"));
  testrun(dtn_item_get(out, "/" "certificate" "/" "ca"
                           "/" "file"));
  testrun(dtn_item_get(out, "/" "certificate" "/" "ca"
                           "/" "path"));

  testrun(dtn_item_is_null(dtn_item_get(out, "/" "path")));
  testrun(dtn_item_is_null(dtn_item_get(out, "/" "name")));
  testrun(
      dtn_item_is_null(dtn_item_get(out, "/" "certificate" "/" "key")));
  testrun(dtn_item_is_null(
      dtn_item_get(out, "/" "certificate" "/" "file")));
  testrun(dtn_item_is_null(dtn_item_get(
      out, "/" "certificate" "/" "ca" "/" "file")));
  testrun(dtn_item_is_null(dtn_item_get(
      out, "/" "certificate" "/" "ca" "/" "path")));
  ...
  testrun(0 ==
          strcmp("1", dtn_item_get_string(dtn_item_get(out, "/" "name"))));
  testrun(0 ==
          strcmp("2", dtn_item_get_string(dtn_item_get(out, "/" "path"))));
  testrun(0 == strcmp("3", dtn_item_get_string(dtn_item_get(
                               out, "/" "certificate" "/" "file"))));
  testrun(0 == strcmp("4", dtn_item_get_string(dtn_item_get(
                               out, "/" "certificate" "/" "key"))));
  testrun(0 == strcmp("5", dtn_item_get_string(dtn_item_get(
                               out, "/" "certificate" "/" "ca"
                                    "/" "file"))));
  testrun(0 == strcmp("6", dtn_item_get_string(dtn_item_get(
                               out, "/" "certificate" "/" "ca"
                                    "/" "path"))));
  out = dtn_item_free(out);
...
int test_dtn_domain_load() {
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    testrun(0 == strcmp((char *)array[i].config.name.start, "domain3"));

    testrun(0 == strcmp(array[i].config.path, DTN_TEST_RESOURCE_DIR));
    testrun(0 == strcmp(array[i].config.certificate.cert, DTN_TEST_CERT));
    testrun(0 == strcmp(array[i].config.certificate.key, DTN_TEST_CERT_KEY));
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    if (0 != strcmp((char *)array[i].config.name.start, "domain3")) {
      ...
      testrun(0 == strcmp(array[i].config.certificate.ca.file, DTN_TEST_CERT));
      testrun(0 == array[i].config.certificate.ca.path[0]);
    }
  }
...
int test_dtn_domain_array_clean() {
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    testrun(
        strcat(array[i].config.certificate.ca.path, "fonafnafpnpanfpasnfpn"));
  }

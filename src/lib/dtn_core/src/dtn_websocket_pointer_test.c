...
int test_dtn_websocket_generate_secure_websocket_key() {
  ...
  for (size_t i = 0; i < amount; i++) {
    ...
    for (size_t k = 0; k < i; k++) {
      ...
      testrun(0 != memcmp(buffer[k], buffer[i], DTN_WEBSOCKET_SECURE_KEY_SIZE));
    }
  }
...
int test_dtn_websocket_process_handshake_request() {
  ...
  testrun(dtn_http_message_add_header_string(in, DTN_WEBSOCKET_KEY_SECURE,
                                            (char *)key));
  testrun(dtn_http_message_add_header_string(in, DTN_WEBSOCKET_KEY_SECURE_VERSION,
                                            "13"));
  testrun(dtn_http_message_close_header(in));
  testrun(DTN_HTTP_PARSER_SUCCESS == dtn_http_pointer_parse_message(in, NULL));
  ...
  testrun(!dtn_websocket_process_handshake_request(NULL, &out, &is_handshake));
  ...
  header = dtn_http_header_get_unique(out->header, out->config.header.capacity,
                                     DTN_HTTP_KEY_UPGRADE);
  ...
  testrun(dtn_http_message_add_header_string(in, DTN_HTTP_KEY_HOST, "host"));
  testrun(
      dtn_http_message_add_header_string(in, DTN_HTTP_KEY_UPGRADE, "somthing"));
  testrun(dtn_http_message_add_header_string(in, DTN_HTTP_KEY_CONNECTION,
                                            DTN_HTTP_KEY_UPGRADE));
  testrun(dtn_http_message_add_header_string(in, DTN_WEBSOCKET_KEY_SECURE,
                                            (char *)key));
...
int check_parse_payload_length() {
  ...
  for (size_t i = 0; i < 126; i++) {
    ...
    if (i == 0) {
      // Zero content but masking set
      testrun(DTN_WEBSOCKET_PARSER_ERROR == parse_payload_length(frame, NULL));
    } else {
      testrun(DTN_WEBSOCKET_PARSER_SUCCESS == parse_payload_length(frame, NULL));
    }
  }
  ...
  for (size_t i = 0xF000; i < 0xFffff; i += 0x10001) {
    ...
    if (i < 0x10000) {
      testrun(DTN_WEBSOCKET_PARSER_ERROR == parse_payload_length(frame, &next));
      continue;
    }
...
int test_dtn_websocket_parse_frame() {
  ...
  testrun(DTN_WEBSOCKET_PARSER_ERROR == dtn_websocket_parse_frame(NULL, NULL));
  testrun(DTN_WEBSOCKET_PARSER_ERROR == dtn_websocket_parse_frame(NULL, &next));
  testrun(DTN_WEBSOCKET_PARSER_ERROR == dtn_websocket_parse_frame(frame, NULL));
  ...
  testrun(DTN_WEBSOCKET_PARSER_ERROR == dtn_websocket_parse_frame(frame, &next));
...
int test_dtn_websocket_set_data() {
  ...
  testrun(dtn_websocket_set_data(frame, (uint8_t *)data, strlen(data), false));
  testrun(frame->buffer);
  testrun(frame->buffer->capacity == 10);

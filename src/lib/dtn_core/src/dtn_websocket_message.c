...
dtn_http_message *dtn_websocket_upgrade_request(const char *host,
                                                const char *uri,
    ...
    msg = dtn_http_create_request_string(
        ...
        (dtn_http_version){.major = 1, .minor = 1}, DTN_HTTP_METHOD_GET, uri);
    ...
    if (!dtn_http_message_add_header_string(
            msg, DTN_WEBSOCKET_KEY_SECURE_VERSION, DTN_WEBSOCKET_VERSION))
        goto error;
...
bool dtn_websocket_is_upgrade_response(const dtn_http_message *msg,
    ...
    if (0 !=
        memcmp(DTN_WEBSOCKET_KEY, header->value.start, header->value.length))
        goto error;

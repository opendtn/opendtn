...
static bool load_certificate(SSL_CTX *ctx, const dtn_domain_config *config) {
  ...
  if (SSL_CTX_use_certificate_chain_file(ctx, config->certificate.cert) != 1) {
    dtn_log_error("%.*s failed to load certificate "
                 "from %s | error %d | %s",
                 config->name.length,config->name.start, config->certificate.cert, errno, strerror(errno));
    goto error;
  }
  ...
  if (SSL_CTX_use_PrivateKey_file(ctx, config->certificate.key,
    ...
    dtn_log_error("%.*s failed to load key "
                 "from %s | error %d | %s",
                 config->name.length,config->name.start, config->certificate.key, errno, strerror(errno));
    goto error;
  }
  ...
  if (SSL_CTX_check_private_key(ctx) != 1) {
    dtn_log_error("%.*s failure private key for\n"
                 "CERT | %s\n"
                 " KEY | %s",
                 config->name.length,config->name.start, config->certificate.cert,
...
static bool init_tls_context(dtn_domain *domain) {
  ...
  if (domain->context.tls) {
    dtn_log_error("TLS context for domain |%.*s| already set",
                 (int)domain->config.name.length, domain->config.name.start);
    goto error;
  }
...
bool dtn_domain_config_verify(const dtn_domain_config *config) {
  ...
  if (!dtn_dir_access_to_path(config->path)) {
    dtn_log_error("Unsufficient access to path|%s| domain |%.*s|", config->path,
                 (int)config->name.length, config->name.start);
    goto error;
  }
...
static bool add_config_to_array(const char *key, const dtn_item *value, void *data) {
  ...
  container->array[container->next].config = dtn_domain_config_from_item(value);

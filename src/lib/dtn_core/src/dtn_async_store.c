...
static bool invalidate_run(uint32_t id, void *data) {
    ...
    if (!dtn_dict_for_each(self->data.dict, &container,
                           search_timed_out_keys)) {
        dtn_log_error("Failed to search timedout items");
        DTN_ASSERT(1 == 0);
    }

    if (!dtn_list_for_each(container.list, self->data.dict,
                           delete_keys_in_dict)) {
...
reschedule:
    ...
    self->invalidate_timer = dtn_event_loop_timer_set(
        self->config.loop, self->config.limits.invalidate_check_usec, self,
        invalidate_run);
...
dtn_async_store *dtn_async_store_free(dtn_async_store *self) {
    ...
    if (DTN_TIMER_INVALID != self->invalidate_timer) {
        ...
        dtn_event_loop_timer_unset(self->config.loop, self->invalidate_timer,
                                   NULL);
    }

...
static void dummy_userdata_clear(dummy_userdata *data) {
  ...
  data->socket = 0;
  data->listener = 0;
  data->domain = dtn_data_pointer_free(data->domain);
  data->data = dtn_buffer_free(data->data);
  return;
}
...
static bool dummy_io(void *userdata, int connection, const char *domain,
                     const dtn_memory_pointer buffer) {
  ...
  if (domain)
    data->domain = dtn_string_dup(domain);

  data->data = dtn_buffer_create(buffer.length);
  dtn_buffer_set(data->data, buffer.start, buffer.length);
  return true;
}
...
int test_dtn_io_create() {
  ...
  dtn_io_config config = {.loop = loop};

  dtn_io *io = dtn_io_create(config);
...
int test_dtn_io_open_listener() {
  ...
  dtn_io_socket_config socket_config = {
  ...
  int server = dtn_io_open_listener(io, socket_config);
  testrun(-1 != server);

  int client = dtn_socket_create(socket_config.socket, true, NULL);
  dtn_socket_ensure_nonblocking(client);
  ...
  testrun(dtn_io_send(
      io, conn, (dtn_memory_pointer){.start = (uint8_t *)"test", .length = 4}));
  ...
  testrun(dtn_io_close(io, conn));
  testrun(0 == recv(client, buf, size, 0));

  client = dtn_socket_create(socket_config.socket, true, NULL);
  dtn_socket_ensure_nonblocking(client);
  dtn_event_loop_run(loop, DTN_RUN_ONCE);
  ...
  socket_config = (dtn_io_socket_config){
      .socket = dtn_socket_load_dynamic_port(
          (dtn_socket_configuration){.type = TLS, .host = "localhost"}),
      .callbacks.userdata = &data,
      .callbacks.accept = dummy_accept,
      .callbacks.io = dummy_io,
      .callbacks.close = dummy_close,
  ...
  server = dtn_io_open_listener(io, socket_config);
  testrun(-1 != server);
  ...
  strncpy(socket_config.ssl.certificate.cert, DTN_TEST_CERT_ONE, PATH_MAX);
  strncpy(socket_config.ssl.certificate.key, DTN_TEST_CERT_ONE_KEY, PATH_MAX);
  strncpy(socket_config.ssl.ca.file, DTN_TEST_CERT_ONE, PATH_MAX);
  strncpy(socket_config.ssl.ca.client_ca, DTN_TEST_CERT_ONE, PATH_MAX);
  ...
  SSL_CTX_set_client_hello_cb(ctx, dummy_client_hello_cb, NULL);
  testrun(1 == SSL_CTX_load_verify_locations(ctx, DTN_TEST_CERT_ONE, NULL));
  testrun(1 == SSL_CTX_use_certificate_file(ctx, DTN_TEST_CERT_ONE,
                                            SSL_FILETYPE_PEM));
  testrun(1 == SSL_CTX_use_PrivateKey_file(ctx, DTN_TEST_CERT_ONE_KEY,
                                           SSL_FILETYPE_PEM));
...
int test_dtn_io_open_connection() {
  ...
  int server_tcp = dtn_io_open_listener(io, socket_config);
  testrun(-1 != server_tcp);

  int client_tcp = dtn_io_open_connection(
      io, (dtn_io_socket_config){.socket = socket_config.socket,
                                .callbacks = socket_config.callbacks});

  dtn_event_loop_run(loop, DTN_RUN_ONCE);
  testrun(-1 != client_tcp);

  testrun(dtn_io_close(io, client_tcp));
  ...
  dtn_io_socket_config client_config =
  ...
  strncpy(client_config.ssl.certificate.cert, DTN_TEST_CERT_ONE, PATH_MAX);
  strncpy(client_config.ssl.certificate.key, DTN_TEST_CERT_ONE_KEY, PATH_MAX);
  strncpy(client_config.ssl.ca.file, DTN_TEST_CERT_ONE, PATH_MAX);

  int client = dtn_io_open_connection(io, client_config);
  testrun(-1 != client);

  // check autoreconnect

  dtn_io_close(io, server);
  dtn_event_loop_run(loop, DTN_RUN_ONCE);
  ...
  Connection *conn = dtn_dict_get(io->connections, (void *)(intptr_t)client);
  ...
  testrun(dtn_io_send(
      io, client,
      (dtn_memory_pointer){.start = (uint8_t *)"test1234", .length = 8}));
...
int all_tests() {
  ...
  testrun_test(test_dtn_io_create);
  testrun_test(test_dtn_io_free);
  testrun_test(test_dtn_io_open_listener);
  testrun_test(test_dtn_io_open_connection);

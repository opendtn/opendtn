...
dtn_http_pointer_parse_version(const uint8_t *buffer, size_t length,
...
dtn_http_parser_state dtn_http_pointer_parse_request_line(
    ...
    if (!dtn_uri_string_is_valid((char *)request->uri.start,
                                 request->uri.length))
        goto error;
...
dtn_http_pointer_parse_header(const uint8_t *buffer, size_t length,
    ...
    for (i = 0; i < array_size; i++) {
        ...
        state =
            dtn_http_pointer_parse_header_line(ptr, size, &array[i], max, &nxt);
...
static dtn_http_message *message_create(dtn_registered_cache *cache,
    ...
    if (msg->buffer) {
        ...
        if (msg->buffer->capacity < config.buffer.default_size) {
            if (!dtn_buffer_extend(msg->buffer, config.buffer.default_size -
                                                    msg->buffer->capacity))
        ...
        while (size > 0) {
            ...
            if (!end) {
                ...
                if (!validate_tranfer_encoding_grammar(
                        array[index_array].start, array[index_array].length,
                        NULL))
                    goto error;
...
static dtn_http_parser_state parse_body(dtn_http_message *msg, uint8_t **next) {
    ...
    transfer_encoding = dtn_http_header_get_next(
        msg->header, msg->config.header.capacity, &index_transfer,
        DTN_HTTP_KEY_TRANSFER_ENCODING);
...
dtn_http_parser_state dtn_http_pointer_parse_message(dtn_http_message *msg,
    ...
    dtn_http_parser_state state = dtn_http_pointer_parse_status_line(
...
dtn_http_message *dtn_http_create_request(dtn_http_message_config config,
    ...
    ssize_t bytes =
        snprintf((char *)msg->buffer->start, msg->buffer->capacity,
                 ...
                 request.method.start, (int)request.uri.length,
                 request.uri.start, version.major, version.minor);
...
bool dtn_http_message_add_header(dtn_http_message *msg,
    ...
    if (DTN_HTTP_PARSER_SUCCESS != dtn_http_pointer_parse_status_line(
...
const uint8_t *dtn_http_pointer_find_item_in_comma_list(const uint8_t *start,
    ...
    while (next) {
        ...
        if (!dtn_http_pointer_parse_comma_list_item(
...
bool dtn_http_message_add_content_type(dtn_http_message *msg, const char *mime,
    ...
    if (charset) {
        ...
        if (!dtn_http_message_add_header_string(msg, DTN_HTTP_KEY_CONTENT_TYPE,
                                                buf))
            goto error;

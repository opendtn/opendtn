...
*//**
        ...
        ------------------------------------------------------------------------
*/
#include "../include/dtn_http_pointer.h"
...
#include "../include/dtn_imf.h"
#include <dtn_base/dtn_constants.h>
...
static bool http_is_token(uint8_t item) {
  ...
  if (http_is_control(item))
    return false;

  if (http_is_separator(item))
    return false;
...
static const uint8_t *http_offset_linear_whitespace(const uint8_t *start,
  ...
  if (http_is_linear_whitespace(start, length))
    return start + 3;
...
static bool http_is_quoted_pair(const uint8_t *start, size_t length) {
  ...
  if (http_is_whitespace(start[1]))
    return true;

  if (http_is_control(start[1]))
    return false;
...
static bool http_is_qd_char(const uint8_t item) {
  ...
  */
  ...
  if ((item >= 0x23) && (item <= 0x7E))
    return true;

  if (http_is_whitespace(item))
    return true;
...
static bool http_is_quoted_string(const uint8_t *start, size_t length) {
  ...
  while (open > 0) {
    ...
    if (!http_is_quoted_pair(ptr, open))
      goto error;
...
dtn_http_parser_state dtn_http_pointer_parse_version(const uint8_t *buffer,
  ...
  if (0 == length)
    return DTN_HTTP_PARSER_PROGRESS;
  ...
  if (0 != strncasecmp((char *)ptr, "HTTP/", len))
    goto error;
  ...
  return DTN_HTTP_PARSER_SUCCESS;

error:
  return DTN_HTTP_PARSER_ERROR;
}
...
dtn_http_parser_state dtn_http_pointer_parse_status_line(const uint8_t *buffer,
  ...
  dtn_http_parser_state state =
      dtn_http_pointer_parse_version(ptr, size, version);
  ...
  // parse phrase
  start = ptr + 13;
  end = (uint8_t *)dtn_string_find((char *)start, size - (start - ptr) + 1,
                                  "\r\n", 2);
  if (!end)
    return DTN_HTTP_PARSER_PROGRESS;
  ...
  if (!http_is_reason_phrase(start, len))
    goto error;
...
dtn_http_parser_state dtn_http_pointer_parse_request_line(
    const uint8_t *buffer, size_t length, dtn_http_request *request,
    dtn_http_version *version, uint32_t max_method_name, uint8_t **next) {
  ...
  const uint8_t *lineend =
      (uint8_t *)dtn_string_find((char *)buffer, length, "\r", 1);
  ...
  if (!dtn_uri_string_is_valid((char *)request->uri.start, request->uri.length))
    goto error;
  ...
  switch (dtn_http_pointer_parse_version(ptr, size, version)) {
...
dtn_http_pointer_parse_header(const uint8_t *buffer, size_t length,
                             dtn_http_header *array, size_t array_size,
  ...
  for (i = 0; i < array_size; i++) {
    ...
    state = dtn_http_pointer_parse_header_line(ptr, size, &array[i], max, &nxt);
...
dtn_http_parser_state dtn_http_pointer_parse_header_line(const uint8_t *buffer,
  ...
  if (!http_is_token_string(line->name.start, line->name.length))
    goto error;
  ...
  lineend = (uint8_t *)dtn_string_find((char *)ptr, size, "\r\n", 2);
  if (!lineend) {
...
const dtn_http_header *dtn_http_header_get_next(const dtn_http_header *array,
  ...
  for (size_t i = *index; i < size; i++) {
    ...
    if (0 ==
        strncasecmp(name, (char *)header->name.start, header->name.length)) {
      *index = i + 1;
      return header;
    }
  }
...
const dtn_http_header *dtn_http_header_get_unique(const dtn_http_header *array,
  ...
  size_t index = 0;
  header = dtn_http_header_get_next(array, size, &index, name);
  if (!header)
    goto error;

  if (dtn_http_header_get_next(array, size, &index, name))
    goto error;
...
const dtn_http_header *dtn_http_header_get(const dtn_http_header *array,
  ...
  size_t index = 0;
  return dtn_http_header_get_next(array, size, &index, name);
}

/*----------------------------------------------------------------------------*/

void dtn_http_message_enable_caching(size_t capacity) {

  dtn_registered_cache_config cfg = {

      .capacity = capacity,
      .item_free = dtn_http_message_free_uncached,

  };

  g_cache = dtn_registered_cache_extend("http_message", cfg);
}

/*----------------------------------------------------------------------------*/

static dtn_http_message *message_create(dtn_registered_cache *cache,
                                       dtn_http_message_config config) {

  dtn_http_message *msg = dtn_registered_cache_get(cache);
  ...
  if (!msg) {
    ...
    msg = calloc(1, sizeof(dtn_http_message) +
                        config.header.capacity * sizeof(dtn_http_header));

    if (!msg)
      goto error;

    msg->magic_byte = DTN_HTTP_MESSAGE_MAGIC_BYTE;
  }

  DTN_ASSERT(dtn_http_message_cast(msg));

  /* Perform a (re)init to zero in any case */

  for (size_t i = 0; i < config.header.capacity; i++) {
    msg->header[i] = (dtn_http_header){0};
  }

  if (msg->buffer) {

    DTN_ASSERT(dtn_buffer_cast(msg->buffer));

    if (msg->buffer->capacity < config.buffer.default_size) {
      if (!dtn_buffer_extend(msg->buffer,
                            config.buffer.default_size - msg->buffer->capacity))
        msg->buffer = dtn_buffer_free(msg->buffer);
    }

    dtn_buffer_clear(msg->buffer);

  } else {

    msg->buffer = dtn_buffer_create(config.buffer.default_size);
    if (!msg->buffer)
      goto error;
  }

  msg->version = (dtn_http_version){0};
  msg->request = (dtn_http_request){0};
  msg->status = (dtn_http_status){0};
  msg->body = (dtn_memory_pointer){0};

  msg->config = config;

  return msg;
error:
  msg = dtn_http_message_free_uncached(msg);
  return NULL;
}
...
dtn_http_message_config_init(dtn_http_message_config config) {
...
dtn_http_message *dtn_http_message_create(dtn_http_message_config config) {

  config = dtn_http_message_config_init(config);

  dtn_http_message *msg = message_create(g_cache, config);

  if (!msg) {
    dtn_log_error("Failed to create a HTTP message structure");
    return NULL;
  }

  DTN_ASSERT(dtn_http_message_cast(msg));
  DTN_ASSERT(msg);
  DTN_ASSERT(msg->buffer);
...
bool dtn_http_message_clear(dtn_http_message *msg) {

  if (!dtn_http_message_cast(msg))
    return false;
  ...
  if (msg->buffer) {
    ...
    if (msg->buffer->capacity > msg->config.buffer.max_bytes_recache) {
      msg->buffer = dtn_buffer_free_uncached(msg->buffer);
    } else {
      dtn_buffer_clear(msg->buffer);
    }
  }
...
void *dtn_http_message_free_uncached(void *message) {

  dtn_http_message *msg = dtn_http_message_cast(message);
  if (!msg)
    return message;
...
void *dtn_http_message_free(void *message) {

  dtn_http_message *msg = dtn_http_message_cast(message);
  if (!msg)
    return message;

  dtn_http_message_clear(msg);
  msg = dtn_registered_cache_put(g_cache, msg);

  return dtn_http_message_free_uncached(msg);
}

/*----------------------------------------------------------------------------*/

dtn_http_message *dtn_http_message_cast(void *data) {

  if (!data)
    return NULL;

  if (*(uint16_t *)data == DTN_HTTP_MESSAGE_MAGIC_BYTE)
    return (dtn_http_message *)data;
...
static bool validate_tranfer_encoding_grammar(const uint8_t *start,
  ...
  if (!delimiter) {
    ...
    if (!http_is_token_string(start, length))
      goto error;
  ...
  if (length == (size_t)(delimiter - start))
    goto error;
  ...
  if (!http_is_token_string(start, (ptr - start)))
    goto error;
  ...
  while (delimiter) {
    ...
    // key MUST be token
    if (!http_is_token_string(ptr, (last - ptr)))
      goto error;
    ...
    if (!http_is_token_string(ptr, len))
      if (!http_is_quoted_string(ptr, len))
        goto error;
  }
...
dtn_http_parser_state dtn_http_pointer_parse_transfer_encodings(
    const dtn_http_message *msg, dtn_memory_pointer *array, size_t array_size) {
  ...
  for (size_t i = 0; i < array_size; i++) {
    array[i] = (dtn_memory_pointer){0};
  }

  const dtn_http_header *header =
      dtn_http_header_get_next(msg->header, msg->config.header.capacity,
                              &index_header, DTN_HTTP_KEY_TRANSFER_ENCODING);

  if (!header)
    return DTN_HTTP_PARSER_ABSENT;
  ...
  while (header) {
    ...
    while (size > 0) {
      ...
      if (!end) {
        ...
        if (0 == array[index_array].length)
          goto error;

        if (!validate_tranfer_encoding_grammar(array[index_array].start,
                                               array[index_array].length, NULL))
          goto error;

        index_array++;
        header = dtn_http_header_get_next(
            msg->header, msg->config.header.capacity, &index_header,
            DTN_HTTP_KEY_TRANSFER_ENCODING);
      ...
      if (ptr == array[index_array].start)
        goto error;
...
static bool check_chunk_extension_grammar(const uint8_t *start, size_t size) {
  ...
  while (ext) {
    ...
    if (!http_is_token_string(ptr, (del - ptr)))
      goto error;

    if (size <= (size_t)(del - start))
      goto error;
...
static dtn_http_parser_state parse_chunked(dtn_http_message *msg,
                                          uint8_t **next) {

  if (!msg || !next || !msg->body.start)
    goto error;
  ...
  size_t length = msg->buffer->length - (msg->body.start - msg->buffer->start);
  ...
  */
  ...
  int64_t chunk_size =
      dtn_string_parse_hex_digits((char *)ptr, s, (char **)&end);
  ...
  if (ext) {
    ...
    if (!check_chunk_extension_grammar(ext, (lineend - ext)))
      goto error;
  ...
  if (length - 2 < (size_t)(lineend - start))
    return DTN_HTTP_PARSER_PROGRESS;
...
static dtn_http_parser_state parse_with_transfer(dtn_http_message *msg,
                                                uint8_t **next) {

  DTN_ASSERT(msg);
  DTN_ASSERT(next);

  dtn_memory_pointer array[msg->config.transfer.max];
  memset(array, 0, msg->config.transfer.max * sizeof(dtn_memory_pointer));

  dtn_http_parser_state state = dtn_http_pointer_parse_transfer_encodings(
  ...
  case DTN_HTTP_PARSER_OOB:

    dtn_log_error("HTTP header with more than %i transfer "
  ...
  for (i = msg->config.transfer.max - 1; i > 0; i--) {
    ...
    if (0 != strncasecmp("chunked", (char *)array[i].start, array[i].length))
      goto error;
...
static dtn_http_parser_state parse_body(dtn_http_message *msg, uint8_t **next) {
  ...
  const dtn_http_header *content_length = NULL;
  const dtn_http_header *transfer_encoding = NULL;
  ...
  content_length =
      dtn_http_header_get_next(msg->header, msg->config.header.capacity,
                              &index_content, DTN_HTTP_KEY_CONTENT_LENGTH);
  transfer_encoding =
      dtn_http_header_get_next(msg->header, msg->config.header.capacity,
                              &index_transfer, DTN_HTTP_KEY_TRANSFER_ENCODING);
  ...
  if (transfer_encoding) {
    ...
    if (dtn_http_header_get_next(msg->header, msg->config.header.capacity,
                                &index_content, DTN_HTTP_KEY_CONTENT_LENGTH))
      goto error;
...
dtn_http_parser_state dtn_http_pointer_parse_message(dtn_http_message *msg,
                                                   uint8_t **next) {
  ...
  dtn_http_parser_state state =
      dtn_http_pointer_parse_status_line(msg->buffer->start, msg->buffer->length,
                                        &msg->status, &msg->version, &nxt);
  ...
  default:
    ...
    state = dtn_http_pointer_parse_request_line(
        msg->buffer->start, msg->buffer->length, &msg->request, &msg->version,
        msg->config.header.max_bytes_method_name, &nxt);
  }
  ...
  state = dtn_http_pointer_parse_header(
  ...
  case DTN_HTTP_PARSER_OOB:
    ...
    dtn_log_error("HTTP header OOB with %zu max config header fields,"
                 " cannot parse header",
                 msg->config.header.capacity);

    dtn_buffer_dump(stderr, msg->buffer);
    goto error;
...
bool dtn_http_message_shift_trailing_bytes(dtn_http_message *source,
  ...
  new = dtn_http_message_create(source->config);
  if (!new)
    goto error;
  ...
  *dest = new;
  return true;
error:
  new = dtn_http_message_free(new);
  return false;
}

/*----------------------------------------------------------------------------*/

bool dtn_http_message_ensure_open_capacity(dtn_http_message *msg,
                                          size_t capacity) {
  ...
  if (!dtn_buffer_extend(msg->buffer, capacity - open))
    goto error;

done:
  DTN_ASSERT(length == msg->buffer->length);
  return true;
error:
  return false;
}
...
dtn_http_message *dtn_http_create_status(dtn_http_message_config config,
  ...
  dtn_http_message *msg = dtn_http_message_create(config);
  if (!msg)
    goto error;

  if (NULL == status.phrase.start)
    goto error;

  if (0 == status.phrase.length)
    goto error;
  ...
  ssize_t bytes =
      snprintf((char *)msg->buffer->start, msg->buffer->capacity,
               "HTTP/%i.%i %i %.*s\r\n", version.major, version.minor,
               status.code, (int)status.phrase.length, status.phrase.start);
...
dtn_http_message *dtn_http_create_request(dtn_http_message_config config,
  ...
  if (NULL == request.method.start)
    goto error;

  if (NULL == request.uri.start)
    goto error;

  if (0 == request.method.length)
    goto error;

  if (0 == request.uri.length)
    goto error;

  if (request.method.length > msg->config.header.max_bytes_method_name)
    goto error;

  if (request.uri.length > PATH_MAX)
    goto error;
  ...
  ssize_t bytes =
      snprintf((char *)msg->buffer->start, msg->buffer->capacity,
               ...
               request.method.start, (int)request.uri.length, request.uri.start,
               version.major, version.minor);
...
bool dtn_http_message_add_header(dtn_http_message *msg, dtn_http_header header) {
  ...
  if (DTN_HTTP_PARSER_SUCCESS !=
      dtn_http_pointer_parse_status_line(msg->buffer->start, msg->buffer->length,
                                        &msg->status, &msg->version, &ptr))
    if (DTN_HTTP_PARSER_SUCCESS !=
        dtn_http_pointer_parse_request_line(
  ...
  while (true) {
    ...
    bytes = snprintf((char *)ptr, open, "%.*s:%.*s\r\n",
                     (int)header.name.length, header.name.start,
                     (int)header.value.length, header.value.start);
    ...
    if (!dtn_http_message_ensure_open_capacity(msg, open))
      goto error;
...
bool dtn_http_message_close_header(dtn_http_message *msg) {
...
bool dtn_http_message_add_body(dtn_http_message *msg, dtn_memory_pointer body) {
  ...
  if (!dtn_http_message_ensure_open_capacity(msg, body.length))
    goto error;
...
bool dtn_http_message_add_body_string(dtn_http_message *msg, const char *body) {
...
bool dtn_http_pointer_parse_comma_list_item(const uint8_t *start, size_t length,
  ...
  if (!start || (length == 0) || !item || !item_len || !next)
    goto error;
...
const uint8_t *dtn_http_pointer_find_item_in_comma_list(const uint8_t *start,
  ...
  if (!start || (length == 0) || !string || (string_length == 0))
    goto error;
  ...
  while (next) {
    ...
    if (!dtn_http_pointer_parse_comma_list_item(next, length - (next - start),
                                               &item, &len, &next))
      goto error;
...
bool dtn_http_is_request(const dtn_http_message *msg, const char *method) {
...
dtn_http_message *dtn_http_status_not_found(const dtn_http_message *src) {
  ...
  out = dtn_http_create_status_string(src->config, src->version, 404,
                                     DTN_HTTP_NOT_FOUND);

  if (!dtn_http_message_set_date(out))
    goto error;
  ...
  if (!snprintf(buffer, size,
                ...
                "File %.*s not found."
                "</body>",
                (int)src->request.uri.length, (char *)src->request.uri.start))
    goto error;

  if (!dtn_http_message_set_content_length(out, strlen(buffer)))
    goto error;

  if (!dtn_http_message_add_header_string(out, DTN_HTTP_KEY_CONTENT_TYPE,
                                         "text/html"))
    goto error;
  ...
  if (!dtn_http_message_close_header(out))
    goto error;

  if (!dtn_http_message_add_body_string(out, buffer))
    goto error;

  return out;
error:
  dtn_http_message_free(out);
  return NULL;
}

/*----------------------------------------------------------------------------*/

dtn_http_message *dtn_http_status_forbidden(const dtn_http_message *src) {

  dtn_http_message *out = NULL;
  if (!src)
    goto error;

  out = dtn_http_create_status_string(src->config, src->version, 403,
                                     DTN_HTTP_FORBIDDEN);
...
bool dtn_http_message_set_date(dtn_http_message *msg) {
  ...
  char timestamp[35] = {0};
  if (!dtn_imf_write_timestamp(timestamp, 35, NULL))
    goto error;

  if (!dtn_http_message_add_header_string(msg, DTN_HTTP_KEY_DATE, timestamp))
    goto error;
...
bool dtn_http_message_set_content_length(dtn_http_message *msg, size_t length) {
  ...
  if (!dtn_http_message_add_header_string(msg, DTN_HTTP_KEY_CONTENT_LENGTH,
                                         string))
    goto error;
...
bool dtn_http_message_set_content_range(dtn_http_message *msg, size_t length,
  ...
  if (!dtn_http_message_add_header_string(msg, DTN_HTTP_KEY_CONTENT_RANGE,
                                         string))
    goto error;
...
bool dtn_http_message_add_content_type(dtn_http_message *msg, const char *mime,
                                      const char *charset) {
  ...
  if (charset) {
    ...
    if (!dtn_http_message_add_header_string(msg, DTN_HTTP_KEY_CONTENT_TYPE, buf))
      goto error;

  } else {

    if (!dtn_http_message_add_header_string(msg, DTN_HTTP_KEY_CONTENT_TYPE, mime))
      goto error;
  }
...
/*
bool dtn_http_message_add_transfer_encodings(dtn_http_message *msg,
                                            const dtn_file_format_desc *format) {
  ...
  if (format) {
    ...
    for (size_t i = 0; i < dtn_FILE_EXT_MAX; i++) {

      if (0 == format->desc.ext[i][0])
        break;

      if (0 ==
          strncasecmp("gzip", format->desc.ext[i], dtn_FILE_EXT_STRING_MAX)) {
      ...
      } else if (0 == strncasecmp("compress", format->desc.ext[i],
                                  dtn_FILE_EXT_STRING_MAX)) {

        if (open < 10)
          goto error;

        if (!strcat(buf, "compress;"))
          goto error;

        open -= 9;

      } else if (0 == strncasecmp("deflate", format->desc.ext[i],
                                  dtn_FILE_EXT_STRING_MAX)) {

        if (open < 9)
          goto error;

        if (!strcat(buf, "deflate;"))
          goto error;
  ...
  if (!strcat(buf, "chunked"))
    goto error;

  if (!dtn_http_message_add_header_string(msg, DTN_HTTP_KEY_TRANSFER_ENCODING,
                                         buf))
    goto error;
...
dtn_http_message *dtn_http_message_pop(dtn_buffer **input,
                                     const dtn_http_message_config *config,
                                     dtn_http_parser_state *state) {

  dtn_http_message *msg = NULL;
  dtn_buffer *new_buffer = NULL;
  dtn_http_parser_state s = DTN_HTTP_PARSER_ERROR;

  if (!input || !state || !config)
    goto error;

  dtn_buffer *buffer = *input;
  if (!dtn_buffer_cast(buffer))
    goto error;

  if (0 == buffer->start[0]) {
    s = DTN_HTTP_PARSER_PROGRESS;
    new_buffer = buffer;
    goto done;
  }
  ...
  s = dtn_http_pointer_parse_message(msg, &next);

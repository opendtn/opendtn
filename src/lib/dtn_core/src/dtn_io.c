...
Connection *connection_create(dtn_io *self) {
    ...
    conn->io = self;
    if (!dtn_thread_lock_init(&conn->io_data.lock,
...
static bool load_domains(dtn_io *self) {
    ...
    if (!dtn_domain_load(self->config.domain.path, &self->domain.size,
                         &self->domain.array)) {
        dtn_log_error("failed to load domains from %s",
...
static bool run_reconnect(uint32_t timer, void *data) {
    ...
    while (config) {
        ...
        if (socket < 0)
            dtn_log_error("reconnect attempt to %s:%i failed",
...
dtn_io *dtn_io_free(dtn_io *self) {
    ...
    if (DTN_TIMER_INVALID != self->timer.reconnects) {
        dtn_event_loop_timer_unset(self->config.loop, self->timer.reconnects,
                                   NULL);
        self->timer.reconnects = DTN_TIMER_INVALID;
    }

    if (DTN_TIMER_INVALID != self->timer.timeouts) {
        dtn_event_loop_timer_unset(self->config.loop, self->timer.timeouts,
                                   NULL);
        self->timer.timeouts = DTN_TIMER_INVALID;
    }
    ...
    if (0 == bytes) {
        ...
        if (conn->type == dtn_IO_CLIENT_CONNECTION) {
            ...
            if (conn->config.auto_reconnect) {
                ...
                dtn_io_socket_config *conf =
                    calloc(1, sizeof(dtn_io_socket_config));
...
static bool stream_send(dtn_io *self, Connection *conn) {
    ...
    if (conn->io_data.out.buffer) {
        ...
        if (bytes < 1) {
            ...
            conn->io_data.out.buffer =
                dtn_buffer_free(conn->io_data.out.buffer);
            goto done;
        }
...
static bool tls_perform_handshake(dtn_io *self, Connection *conn) {
    ...
    if (1 == r) {
        ...
        case SSL_ERROR_SSL:
            ...
            dtn_log_error("SSL_ERROR_SSL %s at socket %i", errorstring,
                          conn->socket);
...
static bool load_verify_locations(SSL_CTX *ctx, dtn_io_ssl_config *config) {
    ...
    if (file || path) {
        ...
        if (r != 1) {
            ...
            dtn_log_error("SSL_CTX_load_verify_locations failed | %s",
                          errorstring);
...
int dtn_io_open_listener(dtn_io *self, dtn_io_socket_config config) {
    ...
    if (!dtn_dict_set(self->connections, (void *)(intptr_t)listener, conn,
                      NULL)) {
        conn = dtn_data_pointer_free(conn);
        goto error;
    }
    ...
    if (-1 == socket) {
        ...
        if (config.auto_reconnect) {
            ...
            *conf = config;
            ...
            dtn_log_error("failed to create socket %s:%i %i|%s",
...
dtn_io_config dtn_io_config_from_item(const dtn_item *input) {
    ...
    config.limits.reconnect_interval_usec =
        dtn_item_get_number(dtn_item_get(conf, "/limits/reconnect_usecs"));

    config.limits.timeout_usec =
        dtn_item_get_number(dtn_item_get(conf, "/limits/timeout_usecs"));
...
bool dtn_io_send(dtn_io *self, int socket, const dtn_memory_pointer buffer) {
    ...
    if (!loop->callback.set(loop, socket,
                            DTN_EVENT_IO_IN | DTN_EVENT_IO_ERR |
                                DTN_EVENT_IO_CLOSE | DTN_EVENT_IO_OUT,
                            self, conn->io_data.callback))
        goto error;

...
#define dtn_IO_MAGIC_BYTES 0xf1f0

#define dtn_SSL_MAX_BUFFER 16000 // 16kb SSL buffer max
#define dtn_SSL_ERROR_STRING_BUFFER_SIZE 200
...
static void *connection_free(void *self) {
  ...
  if (conn->timer_id != DTN_TIMER_INVALID) {

    dtn_event_loop_timer_unset(conn->io->config.loop, conn->timer_id, NULL);
  }

  if (conn->config.callbacks.close) {

    conn->config.callbacks.close(conn->config.callbacks.userdata, conn->socket);
  }
  ...
  if (-1 != conn->socket) {
    ...
    dtn_event_loop_unset(conn->io->config.loop, conn->socket, NULL);
  ...
  if (conn->config.auto_reconnect) {
    ...
    dtn_io_socket_config *conf = calloc(1, sizeof(dtn_io_socket_config));

    *conf = conn->config;
    if (!dtn_list_queue_push(conn->io->reconnects, conf)) {

      conf = dtn_data_pointer_free(conf);
      dtn_log_error("failed to enable auto reconnect");
    }
  }
...
static bool tls_init_context(dtn_io *base, dtn_domain *domain) {

  if (!base || !domain)
    goto error;

  if (!domain->context.tls) {
    dtn_log_error("TLS context for |%s| not set", domain->config.name);
    goto error;
  }
...
static bool load_domains(dtn_io *self) {

  DTN_ASSERT(self);

  /* Load domains */
  if (!dtn_domain_load(self->config.domain.path, &self->domain.size,
                      &self->domain.array)) {
    dtn_log_error("failed to load domains from %s", self->config.domain.path);
    goto error;
  }
...
static bool drop_connection(void *val, void *data) {
  ...
  dtn_io *self = dtn_io_cast(data);
  return dtn_dict_del(self->connections, val);
}
...
static bool run_timeout_check(uint32_t timer, void *data) {
  ...
  dtn_io *self = dtn_io_cast(data);
  DTN_ASSERT(timer == self->timer.timeouts);
  self->timer.timeouts = DTN_TIMER_INVALID;

  uint64_t now = dtn_time_get_current_time_usecs();

  self->timer.timeouts = dtn_event_loop_timer_set(
      self->config.loop, self->config.limits.timeout_usec, self,
      run_timeout_check);

  struct container1 container = (struct container1){
      .now = now, .list = dtn_linked_list_create((dtn_list_config){0})};

  dtn_dict_for_each(self->connections, &container, check_connection_timeout);

  dtn_list_for_each(container.list, self, drop_connection);
...
static bool run_reconnect(uint32_t timer, void *data) {
  ...
  dtn_io *self = dtn_io_cast(data);
  DTN_ASSERT(timer == self->timer.reconnects);
  self->timer.reconnects = DTN_TIMER_INVALID;

  dtn_list *reconnects = self->reconnects;
  self->reconnects = dtn_linked_list_create(
      (dtn_list_config){.item.free = dtn_data_pointer_free});

  dtn_io_socket_config *config = dtn_list_queue_pop(reconnects);

  int socket = -1;
  while (config) {

    socket = dtn_io_open_connection(self, *config);
    if (socket < 0)
      dtn_log_error("reconnect attempt to %s:%i failed", config->socket.host,
                   config->socket.port);

    config = dtn_data_pointer_free(config);
    config = dtn_list_queue_pop(reconnects);
  }

  reconnects = dtn_list_free(reconnects);

  self->timer.reconnects = dtn_event_loop_timer_set(
...
static bool init_config(dtn_io_config *config) {
...
dtn_io *dtn_io_create(dtn_io_config config) {
  ...
  size_t size = sizeof(dtn_io);

  self = calloc(1, size);
  if (!self)
    goto error;

  self->magic_bytes = dtn_IO_MAGIC_BYTES;
  self->config = config;
...
dtn_io *dtn_io_free(dtn_io *self) {
  ...
  if (DTN_TIMER_INVALID != self->timer.reconnects) {
    dtn_event_loop_timer_unset(self->config.loop, self->timer.reconnects, NULL);
    self->timer.reconnects = DTN_TIMER_INVALID;
  }

  if (DTN_TIMER_INVALID != self->timer.timeouts) {
    dtn_event_loop_timer_unset(self->config.loop, self->timer.timeouts, NULL);
    self->timer.timeouts = DTN_TIMER_INVALID;
  }
  ...
  self->domain.array =
      dtn_domain_array_free(self->domain.size, self->domain.array);

  self->reconnects = dtn_list_free(self->reconnects);
...
dtn_io *dtn_io_cast(const void *data) {
  ...
  if (*(uint16_t *)data != dtn_IO_MAGIC_BYTES)
    return NULL;
...
static bool close_listener(dtn_io *self, int socket) {
  ...
  dtn_dict_for_each(self->connections, &container, search_listener);

  dtn_list_for_each(container.list, self, drop_connection);

  dtn_dict_del(self->connections, (void *)(intptr_t)socket);
...
static bool stream_recv_unbuffered(dtn_io *self, Connection *conn) {

  DTN_ASSERT(self);
  DTN_ASSERT(conn);

  size_t size = dtn_socket_get_recv_buffer_size(conn->socket);
  uint8_t buffer[size];
  ...
  if (0 == bytes) {
    ...
    if (conn->type == dtn_IO_CLIENT_CONNECTION) {
      ...
      if (conn->config.auto_reconnect) {
        ...
        *conf = conn->config;
        if (!dtn_list_queue_push(self->reconnects, conf)) {
    ...
    dtn_dict_del(self->connections, (void *)(intptr_t)conn->socket);
    return false;
...
static bool stream_send(dtn_io *self, Connection *conn) {
  ...
  if (conn->io_data.out.buffer) {
    ...
    if (bytes < 1) {
      ...
      conn->io_data.out.buffer = dtn_buffer_free(conn->io_data.out.buffer);
      goto done;
    }
    ...
    if (!buffer) {
      ...
      dtn_event_loop *loop = self->config.loop;
...
static bool io_stream(int socket, uint8_t events, void *data) {
  ...
  Connection *conn =
      (Connection *)dtn_dict_get(self->connections, (void *)(intptr_t)socket);
...
static bool tls_perform_handshake(dtn_io *self, Connection *conn) {

  DTN_ASSERT(self);
  DTN_ASSERT(conn);

  char errorstring[dtn_SSL_ERROR_STRING_BUFFER_SIZE];
  int errorcode = -1;
  int n = 0, r = 0;
  ...
  if (1 == r) {
    ...
    if (!hostname) {
      ...
      if (-1 == self->domain.default_domain) {
        ...
        if (!dtn_string_copy(conn->domain,
      ...
      if (!dtn_string_copy(conn->domain, (char *)hostname, PATH_MAX))
        goto error;
    }
    ...
    case SSL_ERROR_SSL:
      ...
      ERR_error_string_n(errorcode, errorstring,
                         dtn_SSL_ERROR_STRING_BUFFER_SIZE);

      dtn_log_error("SSL_ERROR_SSL %s at socket %i", errorstring, conn->socket);
...
static bool io_stream_ssl_send(dtn_io *self, Connection *conn) {
...
static bool io_stream_ssl(int socket, uint8_t events, void *data) {
  ...
  char errorstring[dtn_SSL_ERROR_STRING_BUFFER_SIZE] = {0};
  int errorcode = -1, n = 0;
  Connection *conn = 0;

  uint8_t buffer[dtn_SSL_MAX_BUFFER] = {0};

  dtn_io *self = dtn_io_cast(data);
  if (!self)
    goto error;

  conn = (Connection *)dtn_dict_get(self->connections, (void *)(intptr_t)socket);
  if (!conn)
    goto error;
  ...
  ssize_t bytes = SSL_read(conn->tls.ssl, buffer, dtn_SSL_MAX_BUFFER);
  ...
  if (bytes > 0) {
    ...
    case SSL_ERROR_SYSCALL:
      ...
      dtn_log_error("SSL_ERROR_SYSCALL"
                   "%d | %s",
...
static Connection *accept_stream_base(
    dtn_io *self, Connection *listener, int socket,
  ...
  if (NULL != listener->config.callbacks.accept) {
    ...
    if (!listener->config.callbacks.accept(listener->config.callbacks.userdata,
                                           socket, nfd))
      goto error;
  }
  ...
  if (!dtn_dict_set(self->connections, (void *)(intptr_t)nfd, conn, NULL)) {
    conn = dtn_data_pointer_free(conn);
    goto error;
  }

  conn->type = dtn_IO_CONNECTION;
  conn->socket = nfd;
  ...
  if (!dtn_event_loop_set(self->config.loop, nfd,
    ...
    dtn_dict_del(self->connections, (void *)(intptr_t)nfd);
    goto error;
  }
...
static bool accept_stream(int socket, uint8_t events, void *data) {
  ...
  Connection *conn = accept_stream_base(self, listener_conn, socket, io_stream);
...
static bool load_certificate(SSL_CTX *ctx, dtn_io_ssl_config *config) {

  if (!ctx || !config)
    goto error;

  if (SSL_CTX_use_certificate_chain_file(ctx, config->certificate.cert) != 1) {
  ...
  dtn_log_debug("loaded SSL certificate \n file %s\n key %s\n",
...
static bool load_verify_locations(SSL_CTX *ctx, dtn_io_ssl_config *config) {
  ...
  if (file || path) {
    ...
    if (r != 1) {
      ...
      dtn_log_error("SSL_CTX_load_verify_locations failed | %s", errorstring);
...
int dtn_io_open_listener(dtn_io *self, dtn_io_socket_config config) {
  ...
  case TLS:
    ...
    break;
  default:
    dtn_log_error("Socket type unsupported");
    goto error;
  }

  int listener = dtn_socket_create(config.socket, false, NULL);
  ...
  if (!dtn_dict_set(self->connections, (void *)(intptr_t)listener, conn, NULL)) {
    conn = dtn_data_pointer_free(conn);
    goto error;
  }

  conn->type = dtn_IO_LISTENER;
  ...
  if (!dtn_event_loop_set(self->config.loop, listener,
                         DTN_EVENT_IO_IN | DTN_EVENT_IO_ERR | DTN_EVENT_IO_CLOSE,
                         self, accept_handler))
    goto error;

  dtn_log_debug("created listener %s:%i", config.socket.host,
               config.socket.port);
...
static bool perform_client_handshake_triggered(uint32_t timer_id, void *data) {
  ...
  DTN_ASSERT(timer_id == conn->timer_id);
  conn->timer_id = DTN_TIMER_INVALID;
...
static bool tls_perform_client_handshake(Connection *conn) {
  ...
  DTN_ASSERT(conn->tls.ssl);

  dtn_io *base = conn->io;
  dtn_event_loop *loop = base->config.loop;
...
call_again_later:
  ...
  if (DTN_TIMER_INVALID == conn->timer_id) {
    dtn_log_error("Failed to reenable trigger "
...
error:
  if (conn)
    dtn_dict_del(conn->io->connections, (void *)(intptr_t)conn->socket);
  return false;
}
...
static bool io_ssl_client(int socket, uint8_t events, void *data) {
  ...
  size_t size = dtn_SSL_MAX_BUFFER;
  uint8_t buffer[size];
  memset(buffer, 0, size);
...
static bool init_ssl_client(dtn_io *self, Connection *conn) {
  ...
  if (file || path) {
    ...
    if (r != 1) {
      ...
      ERR_error_string_n(errorcode, errorstring,
                         dtn_SSL_ERROR_STRING_BUFFER_SIZE);
      dtn_log_error("SSL_CTX_load_verify_locations failed "
                   "at socket %i | %s",
    ...
    dtn_log_error("SSL client %i without verify "
  ...
  if (1 != SSL_set_fd(conn->tls.ssl, conn->socket)) {
    dtn_log_error("SSL_set_fd failed "
                 "at socket %i | %s",
                 conn->socket, errorstring);
    goto error;
  }
...
int dtn_io_open_connection(dtn_io *self, dtn_io_socket_config config) {
  ...
  int socket = dtn_socket_create(config.socket, true, NULL);
  if (-1 == socket) {
    ...
    if (config.auto_reconnect) {
      ...
      *conf = config;
      ...
      dtn_log_error("failed to create socket %s:%i %i|%s", config.socket.host,
  ...
  if (!dtn_socket_ensure_nonblocking(socket)) {
    close(socket);
    goto error;
  }
  ...
  if (!dtn_event_loop_set(self->config.loop, socket,
                         DTN_EVENT_IO_IN | DTN_EVENT_IO_ERR | DTN_EVENT_IO_CLOSE,
                         self, conn->io_data.callback)) {
...
dtn_io_config dtn_io_config_from_item(const dtn_item *input) {
  ...
  dtn_io_config config = {0};

  const dtn_item *conf = dtn_item_object_get(input, "io");
  if (!conf)
    conf = input;

  const char *string =
      dtn_item_get_string(dtn_item_get(conf, "/domain/path"));

  if (string)
    strncpy(config.domain.path, string, PATH_MAX);

  config.limits.reconnect_interval_usec = dtn_item_get_number(
      dtn_item_get(conf, "/limits/reconnect_usecs"));
...
bool dtn_io_close(dtn_io *self, int socket) {
  ...
  if (!self)
    goto error;
  return dtn_dict_del(self->connections, (void *)(intptr_t)socket);
...
bool dtn_io_send(dtn_io *self, int socket, const dtn_memory_pointer buffer) {
  ...
  size_t max = dtn_socket_get_send_buffer_size(socket);
  if (max < buffer.length) {

    dtn_buffer *temp = NULL;
  ...
  dtn_buffer *buf = dtn_buffer_create(buffer.length);
  if (!buf)
    goto error;

...
int test_dtn_password_hash_scrypt() {
  ...
  memset(buffer, 0, size);
  testrun(dtn_password_hash_scrypt(out, &len, "password", "NaCl",
                                  (dtn_password_hash_parameter){.workfactor = 2,
                                                               .blocksize = 2,
                                                               .parallel = 1}));
  end = dtn_time_get_current_time_usecs();
  fprintf(stdout, "HASH time %" PRIu64 " usec\n", end - start);
...
int test_dtn_password_hash_pdkdf2() {
  ...
  start = dtn_time_get_current_time_usecs();
  testrun(
      dtn_password_hash_pdkdf2(out, &len, "password", "NaCl",
...
int test_dtn_password_params_to_item() {
  ...
  testrun(0 ==
          dtn_item_get_number(dtn_item_object_get(val, DTN_AUTH_KEY_WORKFACTOR)));
  testrun(0 ==
          dtn_item_get_number(dtn_item_object_get(val, DTN_AUTH_KEY_BLOCKSIZE)));
  testrun(0 ==
          dtn_item_get_number(dtn_item_object_get(val, DTN_AUTH_KEY_PARALLEL)));
...
int test_dtn_password_is_valid() {
  ...
  testrun(dtn_base64_encode((uint8_t *)salt, strlen(salt), &base64_salt_buffer,
...
int test_dtn_password_hash() {
  ...
  out = dtn_password_hash(pass, (dtn_password_hash_parameter){.workfactor = 2048},
                         0);
  ...
  testrun(2048 ==
          dtn_item_get_number(dtn_item_object_get(out, DTN_AUTH_KEY_WORKFACTOR)));
  testrun(dtn_password_is_valid(pass, out));
  out = dtn_item_free(out);

  // set explicit blocksize
  out =
      dtn_password_hash(pass, (dtn_password_hash_parameter){.blocksize = 16}, 0);
  ...
  testrun(16 ==
          dtn_item_get_number(dtn_item_object_get(out, DTN_AUTH_KEY_BLOCKSIZE)));
  testrun(dtn_password_is_valid(pass, out));
  out = dtn_item_free(out);

  // set explicit parallel
  out = dtn_password_hash(pass, (dtn_password_hash_parameter){.parallel = 16}, 0);
  ...
  testrun(16 ==
          dtn_item_get_number(dtn_item_object_get(out, DTN_AUTH_KEY_PARALLEL)));
  testrun(dtn_password_is_valid(pass, out));
  out = dtn_item_free(out);

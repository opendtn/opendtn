...
int test_dtn_webserver_set_debug() {
    ...
    testrun(dtn_webserver_set_debug(self, false));
...
int test_dtn_webserver_enable_domains() {
    ...
    testrun(dtn_item_array_push(domain_config, domain1));
    testrun(dtn_item_array_push(domain_config, domain2));
    testrun(dtn_item_object_set(domain1, "name", dtn_item_string("name1")));
    testrun(dtn_item_object_set(domain1, "path", dtn_item_string("path1")));
    testrun(dtn_item_object_set(domain2, "name", dtn_item_string("name2")));
    testrun(dtn_item_object_set(domain2, "path", dtn_item_string("path2")));
...
int check_websocket() {
    ...
    int client = dtn_socket_create(config.socket, true, NULL);
    testrun(client > 0);
    ...
    // check pong received
    // dtn_dump_binary_as_hex(stdout, (uint8_t*)buffer, 10);
    testrun(2 == bytes);
...
int test_dtn_webserver_config_from_item() {
    ...
    testrun(dtn_item_object_set(overall_config, "webserver", config));
    testrun(dtn_item_object_set(config, "name", dtn_item_string("name")));
    ...
    dtn_webserver_config conf = dtn_webserver_config_from_item(overall_config);
...
int all_tests() {
    ...
    testrun_test(test_dtn_webserver_create);
    testrun_test(test_dtn_webserver_free);
    testrun_test(test_dtn_webserver_set_debug);
    testrun_test(test_dtn_webserver_enable_callback);
    testrun_test(test_dtn_webserver_enable_domains);
    testrun_test(test_dtn_webserver_config_from_item);

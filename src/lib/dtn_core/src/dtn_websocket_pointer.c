...
bool dtn_websocket_generate_secure_accept_key(const uint8_t *key, size_t len,
  ...
  if (!dtn_hash_string(DTN_HASH_SHA1, acceptkey, strlen((char *)acceptkey), &hash,
                      &buf_size))
    return false;
...
static bool is_upgrade_request(const dtn_http_message *msg) {
  ...
  if (0 !=
      strncasecmp((char *)upgrade->value.start, DTN_WEBSOCKET_KEY, ws_key_len))
    goto error;
...
bool dtn_websocket_process_handshake_request(const dtn_http_message *msg,
  ...
  if (0 != strncmp(DTN_WEBSOCKET_VERSION, (char *)sec_ver->value.start,
    ...
    *out = dtn_http_create_status_string(msg->config, msg->version, 426,
    ...
    dtn_http_message_add_header_string(*out, DTN_WEBSOCKET_KEY_SECURE_VERSION,
  ...
  if (!dtn_websocket_generate_secure_accept_key(sec_key->value.start,
...
static dtn_websocket_frame_config config_init(dtn_websocket_frame_config config) {
...
static dtn_websocket_frame *frame_create(dtn_registered_cache *cache,
  ...
  if (frame->buffer) {
    ...
    if (frame->buffer->capacity < frame->config.buffer.default_size)
      if (!dtn_buffer_extend(frame->buffer, frame->config.buffer.default_size -
...
static dtn_websocket_parser_state parse_payload_length(dtn_websocket_frame *frame,
                                                      uint8_t **next) {
...
finish:
  ...
  if (frame->content.start) {
    ...
    if (frame->buffer->length <
        ((frame->content.start - frame->buffer->start) + frame->content.length))
      return DTN_WEBSOCKET_PARSER_PROGRESS;
...
bool dtn_websocket_set_data(dtn_websocket_frame *frame, const uint8_t *data,
  ...
  if (!frame->buffer) {
    ...
    if (!dtn_buffer_extend(frame->buffer, required - frame->buffer->capacity))
      goto error;
  }

...
static int get_day_index(char *string) {
    ...
    for (size_t i = 0; i < 7; i++) {
        ...
        if (0 == strncasecmp(string, days[i], 3))
            return i;
    }
...
static int get_month_index(char *string) {
    ...
    for (size_t i = 0; i < 12; i++) {
        ...
        if (0 == strncasecmp(string, month[i], 3))
            return i;
    }
...
bool dtn_imf_tm_to_timestamp(const struct tm *source, char *buffer, size_t size,
    ...
    if (!source || !buffer || size < 30)
        goto error;

    int r =
        snprintf(buffer, size, "%s, %02d %s %04i %02d:%02d:%02d %s",
                 days[source->tm_wday], source->tm_mday, month[source->tm_mon],
                 1900 + source->tm_year, source->tm_hour, source->tm_min,
                 source->tm_sec, zone ? zone : "GMT");
...
bool dtn_imf_write_timestamp(char *start, size_t size, char **next) {
    ...
    return dtn_imf_tm_to_timestamp(current, start, size, next, NULL);
error:
    return false;
}
...
struct tm dtn_imf_timestamp_to_tm(const char *start, size_t size, char **next) {
    ...
    if (*end != ' ')
        goto error;
    if ((parsed.tm_mday < 0) || (parsed.tm_mday > 31))
        goto error;
    ptr = end + 1;
    ptr = move_folded_white_space(ptr, size - (size_t)(ptr - start));
    if (!ptr)
    ...
    /*
            ...
            dtn_log_debug("time seconds %i | %i %i %i %i %i %i",
                    time_seconds,
                    parsed.tm_mday,
                    parsed.tm_mon,
                    parsed.tm_year,

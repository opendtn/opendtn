...
bool dtn_imf_tm_to_timestamp(const struct tm *source, char *buffer, size_t size,
  ...
  int r = snprintf(
      buffer, size, "%s, %02d %s %04i %02d:%02d:%02d %s", days[source->tm_wday],
      source->tm_mday, month[source->tm_mon], 1900 + source->tm_year,
      source->tm_hour, source->tm_min, source->tm_sec, zone ? zone : "GMT");

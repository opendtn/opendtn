...
*//**
        ...
        as long as the dtn_http_pointer is used.

        Therefore dtn_http_message_create will create a buffer on init
        and the content buffer is managed within dtn_http_message.

        The buffer MAY be unplugged at ANY time using msg->buffer = NULL.
        Whenever the buffer is unplugged, it MUST be freed using dtn_buffer_free
        at some point, to avoid memleaks.

        Intended usage of dtn_http_message :
            ...
            state = dtn_http_pointer_parse_message(msg, &next);

            if (state == dtn_HTTP_PARSER_SUCCESS){
                ...
                if (next == msg->buffer->start + msg->buffer->length)){
                    ...
                    dtn_http_message *next_msg = NULL;
                    dtn_http_message_shift_trailing_bytes(msg, next, &next_msg);
        ...
        NOTE    allocations MAY be minimized with dtn_http_message_enable_caching
                to cache dtn_http_message structures AND contained buffers.

                dtn_http_message_free is a threadsafe implementation to release
                some structures and return them into the cache

        ------------------------------------------------------------------------
*/
#ifndef dtn_http_pointer_h
#define dtn_http_pointer_h
...
#define DTN_HTTP_MESSAGE_MAGIC_BYTE 0xF0D0

/*
 *      METHODS
 */

#define DTN_HTTP_METHOD_GET "GET"
#define DTN_HTTP_METHOD_HEAD "HEAD"
#define DTN_HTTP_METHOD_POST "POST"
#define DTN_HTTP_METHOD_PUT "PUT"
#define DTN_HTTP_METHOD_DELETE "DELETE"
#define DTN_HTTP_METHOD_CONNECT "CONNECT"
#define DTN_HTTP_METHOD_OPTIONS "OPTIONS"
#define DTN_HTTP_METHOD_TRACE "TRACE"

/*
 *      STATUS
 */

#define DTN_HTTP_CONTINUE "Continue"
#define DTN_HTTP_SWITCH_PROTOCOLS "Switching Protocols"

#define DTN_HTTP_OK "OK"
#define DTN_HTTP_CREATED "Created"
#define DTN_HTTP_ACCEPTED "Accepted"
#define DTN_HTTP_NON_AUTH_INFO "Non-Authoritative Information"
#define DTN_HTTP_NO_CONTENT "No Content"
#define DTN_HTTP_RESET_CONTENT "Reset Content"
#define DTN_HTTP_PARTIAL_CONTENT "Partial Content"

#define DTN_HTTP_MULTIPLE_CHOICES "Multiple Choices"
#define DTN_HTTP_MOVED_PERMANENTLY "Moved Permanently"
#define DTN_HTTP_FOUND "Found"
#define DTN_HTTP_SEE_OTHER "See Other"
#define DTN_HTTP_NOT_MODIFIED "Not Modified"
#define DTN_HTTP_USE_PROXY "Use Proxy"
#define DTN_HTTP_TEMPORARY_REDIRECT "Temporary Redirect"

#define DTN_HTTP_BAD_REQUEST "Bad Request"
#define DTN_HTTP_UNAUTHORIZED "Unauthorized"
#define DTN_HTTP_PAYMENT_REDIRECT "Payment Required"
#define DTN_HTTP_FORBIDDEN "Forbidden"
#define DTN_HTTP_NOT_FOUND "Not Found"
#define DTN_HTTP_NOT_ALLOWED "Method Not Allowed"
#define DTN_HTTP_NOT_ACCEPTABLE "Not Acceptable"
#define DTN_HTTP_PROXY_AUTH_REQUIRED "Proxy Authentication Required"
#define DTN_HTTP_REQUEST_TIMEOUT "Request Timeout"
#define DTN_HTTP_CONFLICT "Conflict"
#define DTN_HTTP_GONE "Gone"
#define DTN_HTTP_LENGTH_REQUIRED "Length Required"
#define DTN_HTTP_PRECONDITION_FAILED "Precondition Failed"
#define DTN_HTTP_PAYLOAD_TO_LARGE "Payload Too Large"
#define DTN_HTTP_URI_TO_LONG "URI Too Long"
#define DTN_HTTP_UNSUPPORTED_MEDIA "Unsupported Media Type"
#define DTN_HTTP_RANGE_NOT_SATISFIABLE "Range Not Satisfiable"
#define DTN_HTTP_EXPECTATION_FAILED "Expectation Failed"
#define DTN_HTTP_UPGRADE_REQUIRED "Upgrade Required"

#define DTN_HTTP_INTERNAL_SERVER_ERROR "Internal Server Error"
#define DTN_HTTP_NOT_IMPLEMENTED "Not Implemented"
#define DTN_HTTP_BAD_GATEWAY "Bad Gateway"
#define DTN_HTTP_SERVICE_UNAVAILABLE "Service Unavailable"
#define DTN_HTTP_GATEWAY_TIMEOUT "Gateway Timeout"
#define DTN_HTTP_VERSION_NOT_SUPPORTED "HTTP Version Not Supported"

/*
 *      HEADER FIELDS
 */

#define DTN_HTTP_KEY_TRANSFER_ENCODING "Transfer-Encoding"
#define DTN_HTTP_KEY_CONTENT_LENGTH "Content-Length"
#define DTN_HTTP_KEY_CONTENT_RANGE "Content-Range"
#define DTN_HTTP_KEY_CONTENT_TYPE "Content-Type"
#define DTN_HTTP_KEY_CONNECT "Connect"
#define DTN_HTTP_KEY_CONNECTION "Connection"
#define DTN_HTTP_KEY_UPGRADE "Upgrade"
#define DTN_HTTP_KEY_DATE "Date"
#define DTN_HTTP_KEY_SERVER "Server"
#define DTN_HTTP_KEY_HOST "Host"

#define DTN_HTTP_KEY_LOCATION "Location"

/*
 *      TRANSFER ENCODING
 */

#define DTN_HTTP_CHUNKED "chunked"
#define DTN_HTTP_COMPRESS "compress"
#define DTN_HTTP_DEFLATE "deflate"
#define DTN_HTTP_GZIP "gzip"
...
bool dtn_http_message_clear(dtn_http_message *message);
void *dtn_http_message_free(void *message);
void *dtn_http_message_free_uncached(void *message);
dtn_http_message *dtn_http_message_cast(void *message);
...
/**
    Create some dtn_http_message and set status line in msg->buffer
...
dtn_http_message *dtn_http_status_not_found(const dtn_http_message *source);
dtn_http_message *dtn_http_status_forbidden(const dtn_http_message *source);

/*----------------------------------------------------------------------------*/

/**
    Create some dtn_http_message and set request line in msg->buffer
...
/**
    ...
    @see dtn_http_pointer_parse_message after success of the parsing, to move
    ...
    @param dest     output pointer for new dtn_http_message, containing the
                    additional bytes (if any)

    @NOTE after *dest is set it SHOULD be checked using
    @see dtn_http_pointer_parse_message for grammar compliance

    @NOTE source->buffer will be set to 0 starting at next
    @NOTE on success *dest will be set and MAY be an empty dtn_http_message
...
/**
    ...
    @param version  (mandatory) dtn_http_version to be set
...
/**
    ...
    @param buffer   pointer to start of the buffer to match
    @param length   length of the buffer to match
    @param status   (mandatory) dtn_http_status to be set
    @param version  (mandatory) dtn_http_version to be set

    @returns parsing state of the buffer

    @NOTE dtn_HTTP_PARSER_SUCCESS will be set and phrase will point to the input
   buffer
...
/**
    ...
    @param buffer   pointer to start of the buffer to match
    @param length   length of the buffer to match
    @param request  (mandatory) dtn_http_request to be set
    @param version  (mandatory) dtn_http_version to be set
    @param max_req  (optional) max request length (0 == unlimited)

    @returns parsing state of the buffer
‚
    @NOTE dtn_HTTP_PARSER_SUCCESS will be set and uri and method
...
dtn_http_parser_state dtn_http_pointer_parse_request_line(
    ...
    dtn_http_version *version, uint32_t max_req, uint8_t **next);
...
/**
    ...
    @param buffer   pointer to start of the buffer to match
    @param length   length of the buffer to match
    @param line     (mandatory) dtn_http_header to be set
    @param max      (optional) max line length (0 == unlimited)

    @returns parsing state of the buffer

    @NOTE dtn_HTTP_PARSER_SUCCESS name and value will point to the input buffer
...
/**
    ...
    @param buffer   pointer to start of the buffer to match
    @param length   length of the buffer to match
    @param array    array of dtn_http_header lines
...
/**
    ...
    This function is used in e.g. @see dtn_http_pointer_find_item_in_comma_list
...
/**
    ...
    @param array    some array of dtn_http_header pointers
...
/**
    ...
    @returns
    dtn_HTTP_PARSER_ABSENT if no transfer encoding is used within the header
    dtn_HTTP_PARSER_OOB if more tranfer encodings as size are found

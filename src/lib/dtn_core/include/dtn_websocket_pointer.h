...
*//**
        ...
        as long as the dtn_websocket_frame is used.

        Therefore dtn_websocket_frame_create will create a buffer on init
        and the content buffer is managed within dtn_websocket_frame.
        ...
        Intended usage of dtn_websocket_frame :
            ...
            state = dtn_websocket_parse_frame(msg, &next);
            ...
            if (state == dtn_HTTP_PARSER_SUCCESS){
                ...
                if (next == frame->buffer->start + frame->buffer->length)){
                    ...
                    dtn_websocket_frame *next_frame = NULL;
                    dtn_websocket_shift_trailing_bytes(frame, next, &next_frame);
        ...
        NOTE    allocations MAY be minimized with dtn_websocket_enable_caching
                to cache dtn_websocket_frame structures AND contained buffers.

                dtn_websocket_frame_free is a threadsafe implementation to
                release some structures and return them into the cache


        ------------------------------------------------------------------------
*/
#ifndef dtn_websocket_pointer_h
#define dtn_websocket_pointer_h

#include "dtn_http_pointer.h"
#include <stdbool.h>

#define DTN_WEBSOCKET_MAGIC_BYTE 0xF0DF
#define DTN_WEBSOCKET_SECURE_KEY_SIZE 24
...
#define DTN_WEBSOCKET_VERSION "13"

#define DTN_WEBSOCKET_KEY "websocket"
#define DTN_WEBSOCKET_KEY_UPGRADE "upgrade"
#define DTN_WEBSOCKET_KEY_SECURE "Sec-WebSocket-Key"
#define DTN_WEBSOCKET_KEY_SECURE_VERSION "Sec-WebSocket-Version"
#define DTN_WEBSOCKET_KEY_SECURE_ACCEPT "Sec-WebSocket-Accept"

#define DTN_WEBSOCKET_KEY_SECURE_PROTOCOL "Sec-WebSocket-Protocol"
#define DTN_WEBSOCKET_KEY_SECURE_EXTENSION "Sec-WebSocket-Extension"
...
dtn_websocket_frame *dtn_websocket_frame_cast(void *message);
...
/**
    ...
    @see dtn_websocket_parse_frame after success of the parsing, to move
    ...
    @see dtn_websocket_parse_frame for compliance

    @NOTE source->buffer will be set to 0 starting at next
    @NOTE on success *dest will be set and MAY be an empty dtn_websocket_frame
...
*/
bool dtn_websocket_frame_unmask(dtn_websocket_frame *parsed_frame);
...
/**
    ...
    @param size     size of buffer (min dtn_WEBSOCKET_SECURE_KEY_SIZE)
*/

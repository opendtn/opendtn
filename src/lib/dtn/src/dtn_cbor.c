...
static dtn_dict_config dtn_cbor_dict_config(uint64_t slots) {
    ...
    return (dtn_dict_config){
        ...
        .key.data_function = (dtn_data_function){.clear = cbor_clear,
                                                 .copy = cbor_copy,
                                                 .free = cbor_free,
                                                 .dump = cbor_dump},
        .key.hash = cbor_hash,
        ...
        .value.data_function = (dtn_data_function){.clear = cbor_clear,
                                                   .copy = cbor_copy,
                                                   .free = cbor_free,
                                                   .dump = cbor_dump}};
}
...
dtn_cbor_type dtn_cbor_get_type(const dtn_cbor *self) { return self->type; }
...
static dtn_cbor_match decode_uint(const uint8_t *buffer, size_t size,
...
static dtn_cbor_match decode_int(const uint8_t *buffer, size_t size,
                                 dtn_cbor **out, uint8_t **next) {
...
static dtn_cbor_match decode_text_string(const uint8_t *buffer, size_t size,
                                         dtn_cbor **out, uint8_t **next) {
...
static dtn_cbor_match decode_utf8_string(const uint8_t *buffer, size_t size,
                                         dtn_cbor **out, uint8_t **next) {
    ...
    if (self->nbr_uint > 0)
        if (!dtn_utf8_validate_sequence(self->bytes, self->nbr_uint))
            goto error;
...
static dtn_cbor_match decode_array(const uint8_t *buffer, size_t size,
                                   dtn_cbor **out, uint8_t **next) {
    ...
    case 0x9F:
        ...
        while ((int64_t)(size - (ptr - buffer) - 1) > 0) {
            ...
            dtn_cbor_match match =
                dtn_cbor_decode(ptr, (size - (ptr - buffer) - 1), &item, &ptr);
...
static dtn_cbor_match decode_map(const uint8_t *buffer, size_t size,
                                 dtn_cbor **out, uint8_t **next) {
    ...
    case 0xBF:
        ...
        while ((int64_t)(size - (ptr - buffer) - 1) > 0) {
            ...
            dtn_cbor_match match =
                dtn_cbor_decode(ptr, (size - (ptr - buffer) - 1), &key, &ptr);
            ...
            match =
                dtn_cbor_decode(ptr, (size - (ptr - buffer) - 1), &val, &ptr);
    ...
    for (uint64_t i = 1; i <= map_items; i++) {
        ...
        dtn_cbor_match match =
            dtn_cbor_decode(ptr, size - (ptr - buffer), &key, &ptr);
        ...
        match = dtn_cbor_decode(ptr, size - (ptr - buffer), &val, &ptr);
...
static dtn_cbor_match decode_tag(const uint8_t *buffer, size_t size,
                                 dtn_cbor **out, uint8_t **next) {
...
static dtn_cbor_match decode_simple(const uint8_t *buffer, size_t size,
                                    dtn_cbor **out, uint8_t **next) {
...
static uint64_t cbor_array_encoding_size(const dtn_cbor *self) {
    ...
    for (uint64_t i = 1; i <= dtn_list_count(self->data); i++) {
        ...
        items += dtn_cbor_encoding_size(dtn_list_get(self->data, i));
    }
...
static uint64_t cbor_map_encoding_size(const dtn_cbor *self) {
    ...
    items = 0;
    if (!dtn_dict_for_each(self->data, &items, count_item_size))
        goto error;
...
static bool encode_uint(const dtn_cbor *self, uint8_t *buffer, size_t size,
                        uint8_t **next) {

    if (!self || !buffer || size < 1 || !next)
        goto error;

    if (self->type != DTN_CBOR_UINT64)
        goto error;
...
static bool encode_int(const dtn_cbor *self, uint8_t *buffer, size_t size,
                       uint8_t **next) {

    if (!self || !buffer || size < 1 || !next)
        goto error;

    if (self->type != DTN_CBOR_INT64)
        goto error;
...
static bool encode_string(const dtn_cbor *self, uint8_t *buffer, size_t size,
                          uint8_t **next) {

    if (!self || !buffer || size < 1 || !next)
        goto error;

    if (self->type != DTN_CBOR_STRING)
        goto error;
...
static bool encode_uft8(const dtn_cbor *self, uint8_t *buffer, size_t size,
                        uint8_t **next) {

    if (!self || !buffer || size < 1 || !next)
        goto error;

    if (self->type != DTN_CBOR_UTF8)
        goto error;
...
static bool encode_array(const dtn_cbor *self, uint8_t *buffer, size_t size,
                         uint8_t **next) {

    if (!self || !buffer || size < 1 || !next)
        goto error;

    if (self->type != DTN_CBOR_ARRAY)
        goto error;
    ...
    for (uint64_t i = 1; i <= dtn_list_count(self->data); i++) {
        ...
        if (!dtn_cbor_encode(item, ptr, size - (ptr - buffer), &ptr))
            goto error;
    }
...
static bool write_items(const void *key, void *val, void *data) {
    ...
    if (!dtn_cbor_encode(k, c->ptr, c->size, &c->ptr))
        goto error;

    if (!dtn_cbor_encode(v, c->ptr, c->size, &c->ptr))
        goto error;
...
static bool encode_map(const dtn_cbor *self, uint8_t *buffer, size_t size,
                       uint8_t **next) {

    if (!self || !buffer || size < 1 || !next)
        goto error;

    if (self->type != DTN_CBOR_MAP)
        goto error;
    ...
    if (!dtn_dict_for_each(self->data, &container, write_items))
        goto error;
...
static bool encode_date_time(const dtn_cbor *self, uint8_t *buffer, size_t size,
                             uint8_t **next) {

    if (!self || !buffer || size < 1 || !next)
        goto error;

    if (self->type != DTN_CBOR_DATE_TIME)
        goto error;
...
static bool encode_date_time_epoch(const dtn_cbor *self, uint8_t *buffer,
                                   size_t size, uint8_t **next) {

    if (!self || !buffer || size < 1 || !next)
        goto error;

    if (self->type != DTN_CBOR_DATE_TIME_EPOCH)
        goto error;
...
static bool encode_ubignum(const dtn_cbor *self, uint8_t *buffer, size_t size,
                           uint8_t **next) {

    if (!self || !buffer || size < 1 || !next)
        goto error;

    if (self->type != DTN_CBOR_UBIGNUM)
        goto error;
...
static bool encode_ibignum(const dtn_cbor *self, uint8_t *buffer, size_t size,
                           uint8_t **next) {

    if (!self || !buffer || size < 1 || !next)
        goto error;

    if (self->type != DTN_CBOR_IBIGNUM)
        goto error;
...
static bool encode_fraction(const dtn_cbor *self, uint8_t *buffer, size_t size,
                            uint8_t **next) {

    if (!self || !buffer || size < 1 || !next)
        goto error;

    if (self->type != DTN_CBOR_DEC_FRACTION)
        goto error;
    ...
    for (uint64_t i = 0; i < dtn_cbor_array_count(self->data); i++) {
        ...
        items += dtn_cbor_encoding_size(dtn_cbor_array_get(self->data, i));
    }
...
static bool encode_bigfloat(const dtn_cbor *self, uint8_t *buffer, size_t size,
                            uint8_t **next) {

    if (!self || !buffer || size < 1 || !next)
        goto error;

    if (self->type != DTN_CBOR_BIGFLOAT)
        goto error;
...
static bool encode_tag(const dtn_cbor *self, uint8_t *buffer, size_t size,
                       uint8_t **next) {
    ...
    case 0xD7:
        ...
        if (!dtn_cbor_encode(self->data, buffer + 1, size - 1, next))
            goto error;
    ...
    case 0xd9:
        ...
        if (!dtn_cbor_encode(self->data, buffer + 3, size - 3, next))
            goto error;
    ...
    case 0xdA:
        ...
        if (!dtn_cbor_encode(self->data, buffer + 5, size - 5, next))
            goto error;
    ...
    case 0xdB:
        ...
        if (!dtn_cbor_encode(self->data, buffer + 9, size - 9, next))
            goto error;
...
static bool encode_simple(const dtn_cbor *self, uint8_t *buffer, size_t size,
                          uint8_t **next) {

    if (!self || !buffer || size < 1 || !next)
        goto error;

    if (self->type != DTN_CBOR_SIMPLE)
        goto error;
...
static bool encode_float(const dtn_cbor *self, uint8_t *buffer, size_t size,
                         uint8_t **next) {
...
static bool encode_double(const dtn_cbor *self, uint8_t *buffer, size_t size,
                          uint8_t **next) {
...
bool dtn_cbor_encode_array_of_indefinite_length(const dtn_cbor *value,
    ...
    for (uint64_t i = 0; i < count; i++) {
        ...
        if (!dtn_cbor_encode(dtn_cbor_array_get(value, i), ptr,
                             size - (ptr - buffer), &ptr))
            goto error;
    }
...
bool dtn_cbor_is_map(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_MAP)
        return false;
    return true;
}
...
bool dtn_cbor_map_set_string(dtn_cbor *map, const char *key, dtn_cbor *val) {
    ...
    if (!dtn_dict_set(map->data, k, val, NULL))
        goto error;
    return true;
error:
    k = cbor_free(k);
...
dtn_cbor *dtn_cbor_array() {
    ...
    out->data =
        dtn_linked_list_create((dtn_list_config){.item.copy = cbor_copy,
...
bool dtn_cbor_is_array(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_ARRAY)
        return false;
    return true;
}
...
bool dtn_cbor_array_set(dtn_cbor *self, uint64_t index, dtn_cbor *data) {
...
bool dtn_cbor_is_string(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_STRING)
        return false;
    return true;
}
...
bool dtn_cbor_set_byte_string(dtn_cbor *self, const uint8_t *byte,
                              size_t size) {
...
dtn_cbor *dtn_cbor_utf8(const uint8_t *buffer, size_t size) {
    ...
    if (!dtn_utf8_validate_sequence(buffer, size))
        goto error;
...
bool dtn_cbor_is_utf8(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_UTF8)
        return false;
    return true;
}
...
dtn_cbor *dtn_cbor_true() { return dtn_cbor_create(DTN_CBOR_TRUE); }

/*----------------------------------------------------------------------------*/

bool dtn_cbor_is_true(const dtn_cbor *self) {

    if (self->type == DTN_CBOR_TRUE)
        return true;
    return false;
}
...
dtn_cbor *dtn_cbor_false() { return dtn_cbor_create(DTN_CBOR_FALSE); }

/*----------------------------------------------------------------------------*/

bool dtn_cbor_is_false(const dtn_cbor *self) {

    if (self->type == DTN_CBOR_FALSE)
        return true;
    return false;
}
...
dtn_cbor *dtn_cbor_null() { return dtn_cbor_create(DTN_CBOR_NULL); }

/*----------------------------------------------------------------------------*/

bool dtn_cbor_is_null(const dtn_cbor *self) {

    if (self->type == DTN_CBOR_NULL)
        return true;
    return false;
}
...
dtn_cbor *dtn_cbor_undef() { return dtn_cbor_create(DTN_CBOR_UNDEF); }

/*----------------------------------------------------------------------------*/

bool dtn_cbor_is_undef(const dtn_cbor *self) {

    if (self->type == DTN_CBOR_UNDEF)
        return true;
    return false;
}
...
bool dtn_cbor_is_uint(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_UINT64)
        return false;
    return true;
}
...
bool dtn_cbor_is_int(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_INT64)
        return false;
    return true;
}
...
bool dtn_cbor_is_time(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_DATE_TIME)
        return false;
    return true;
}
...
bool dtn_cbor_is_time_epoch(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_DATE_TIME_EPOCH)
        return false;
    return true;
}
...
bool dtn_cbor_is_ubignum(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_UBIGNUM)
        return false;
    return true;
}
...
bool dtn_cbor_is_ibignum(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_IBIGNUM)
        return false;
    return true;
}
...
bool dtn_cbor_is_dec_fraction(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_DEC_FRACTION)
        return false;
    return true;
}
...
bool dtn_cbor_is_bigfloat(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_BIGFLOAT)
        return false;
    return true;
}
...
bool dtn_cbor_is_tag(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_TAG)
        return false;
    return true;
}
...
bool dtn_cbor_is_simple(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_SIMPLE)
        return false;
    return true;
}
...
bool dtn_cbor_is_float(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_FLOAT)
        return false;
    return true;
}
...
bool dtn_cbor_is_double(const dtn_cbor *self) {
    ...
    if (!self || self->type != DTN_CBOR_DOUBLE)
        return false;
    return true;
}

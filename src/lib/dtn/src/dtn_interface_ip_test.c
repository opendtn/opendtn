...
int test_dtn_interface_ip_create() {
    ...
    testrun(self->buffer) testrun(DTN_TIMER_INVALID != self->timer.link_check);
...
static void dummy_io(void *userdata, const dtn_socket_data *remote,
...
int check_io() {
    ...
    testrun(dtn_socket_fill_sockaddr_storage(&sa, AF_INET, config.socket.host,
                                             config.socket.port));
    ...
    while (-1 == bytes) {
        ...
        bytes = sendto(client, buffer, next - buffer, 0, (struct sockaddr *)&sa,
                       sa_len);
    ...
    while (-1 == bytes) {
        ...
        bytes =
            sendto(client, buffer + 10, 20, 0, (struct sockaddr *)&sa, sa_len);
        dtn_event_loop_run(loop, DTN_RUN_ONCE);
    }
    ...
    while (-1 == bytes) {
        ...
        bytes =
            sendto(client, buffer + 15, 20, 0, (struct sockaddr *)&sa, sa_len);
        dtn_event_loop_run(loop, DTN_RUN_ONCE);
    }
    ...
    while (-1 == bytes) {
        ...
        bytes = sendto(client, buffer, one_bundle, 0, (struct sockaddr *)&sa,
                       sa_len);
        dtn_event_loop_run(loop, DTN_RUN_ONCE);
    }
    ...
    while (-1 == bytes) {
        ...
        bytes = sendto(client, "test1234567890", 14, 0, (struct sockaddr *)&sa,
                       sa_len);
        dtn_event_loop_run(loop, DTN_RUN_ONCE);
    }
    ...
    while (-1 == bytes) {
        ...
        bytes =
            sendto(client, buffer + 10, 10, 0, (struct sockaddr *)&sa, sa_len);
        dtn_event_loop_run(loop, DTN_RUN_ONCE);
    }
    ...
    while (-1 == bytes) {
        ...
        bytes = sendto(client, buffer + 20, next - buffer - 20, 0,

...
void *dtn_ipn_free(void *self){
    ...
    if (!dtn_ipn_clear(uri)) goto error;
...
dtn_ipn *dtn_ipn_decode(const char *string){
    ...
    if (del2 - del1 < 2) goto error;
    if (size - (del2 - string) == 1) goto error;

    char *ptr = del1 + 1;
    while(ptr < del2){

        if (!isdigit(ptr[0])) goto error;
        ptr++;
    ...
    if (!strncpy(self->scheme, string, (del1 - string))) goto error;

    self->node = calloc( (del2 - del1) + 1, sizeof(char));
    if (!self->node) goto error;
    if (!strncpy(self->node, del1 + 1 , (del2 - del1)-1 )) goto error;

    self->service = calloc( size - (del2 - string) + 1, sizeof(char));
    if (!self->service) goto error;
    if (!strncpy(self->service, del2 + 1 , (size - (del2 - string) -1))) goto error;

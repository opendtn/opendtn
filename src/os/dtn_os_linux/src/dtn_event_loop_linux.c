/***
        ...
        ------------------------------------------------------------------------
*//**
        @file           dtn_event_loop_linux.c
        @author         Michael Beer

        @date           2019-09-04

        @ingroup        dtn_event_loop_linux

        @brief          Linux version of dtn_event_loop_poll
...
#include "../include/dtn_event_loop_linux.h"
...
static uint32_t to_epoll_flags(uint8_t dtn_flag) {
...
static bool register_fd_with_epoll(Loop *loop,
    ...
    if (0 == callback) {
        dtn_log_error("Wont register without callback");
        goto error;
    }
    ...
    if (0 == cb) {
        dtn_log_error("Could not register socket %i on loop", fd);
        goto error;
    }

    DTN_ASSERT(0 != cb);
    DTN_ASSERT((0 > cb->fd) || (fd == cb->fd));
    ...
    if (epoll_ctl(loop->epoll_fd, EPOLL_CTL_ADD, fd, &ev) == -1) {
        ...
        dtn_log_error("Could not add new fd to epoll: %i", fd);
        goto error;
    }
...
static bool cache_timer_fd_unsafe(Loop *loop,
    ...
    DTN_ASSERT(0 != loop->timers_available.timers);
...
static bool release_timer_fd_unsafe(Loop *loop, int fd, void **userdata) {
    ...
    if (fd != cb->fd) {
        dtn_log_error("Tried to unregister non-existent timer %i ", fd);
        return false;
    }
    ...
    if (0 != timerfd_settime(fd, 0, &none, 0)) {
        dtn_log_warning("Could not disarm timer %i", fd);
    }
...
static bool callback_for_timer(int fd, uint8_t events, void *data) {
    ...
    if (0 == tdata) {
        dtn_log_error("Timer callback did not receive valid timer data!");
        return false;
    }
    ...
    if (0 == loop) {
        dtn_log_error("Expected event loop as argument");
        close(fd);
        return false;
    }
    ...
    if (8 != read(fd, &expired_count, sizeof(expired_count))) {
        dtn_log_error("Could not read expiration counter for timer %i", fd);
        goto error;
    }
...
static bool acquire_timer_data_unsafe(Loop *loop,
    ...
    if (0 > loop->epoll_fd) {
        dtn_log_error("Loop does not seem to run? epoll fd not set");
        goto error;
    }
...
static bool trigger_callback(Loop *loop, struct epoll_event *ep_event) {
    ...
    if ((0 == cb) || (0 > cb->fd)) {
        dtn_log_error("No callback for fd %i", fd);
        return false;
    }

    if (0 == cb->callback) {
        dtn_log_error("Invalid callback: Function pointer 0");
        goto error;
    }
    ...
    if ((ep_event->events & EPOLLIN) || (ep_event->events & EPOLLPRI)) {
        ...
        events |= DTN_EVENT_IO_IN;
    }

    if (ep_event->events & EPOLLOUT) {

        events |= DTN_EVENT_IO_OUT;
    }

    if (ep_event->events & EPOLLHUP) {

        events |= DTN_EVENT_IO_CLOSE;
        closed = true;
    }

    if (ep_event->events & EPOLLERR) {

        events |= DTN_EVENT_IO_ERR;
    }
...
static bool loop_init(Loop *loop, dtn_event_loop_config config) {
    ...
    if (0 == loop->callbacks) {
        dtn_log_error("Failed to allocate bytes for callbacks.");
        goto error;
    }
    ...
    if (0 > loop->epoll_fd) {
        dtn_log_error("Could not create epoll fd");
        goto error;
    }
...
bool impl_event_loop_linux_run(dtn_event_loop *self, uint64_t max_usecs) {
    ...
    if (0 > epfs) {
        dtn_log_error("epoll not initialized properly");
        goto error;
    }

    loop->running = true;

    if (max_usecs == DTN_RUN_ONCE) {
        loop->running = false;
        max_usecs = 0;
    }

    uint64_t start_usecs = dtn_time_get_current_time_usecs();
    ...
    do {
        ...
        if (0 > num_ready) {
            dtn_log_error("I/O error occured: %s\n", strerror(errno));
            goto error;
        }
...
bool impl_event_loop_linux_callback_set(dtn_event_loop *self,
    ...
    if (!dtn_socket_ensure_nonblocking(fd)) {
        close(fd);
        goto error;
    }
...
uint32_t impl_event_loop_linux_timer_set(dtn_event_loop *self,
    ...
    if ((0 > timer_fd) || (0 == tdata)) {
        dtn_log_error("could not acquire a new timer");
        goto error;
    }
    ...
    if (0 != timerfd_settime(timer_fd, 0, &tspec, 0)) {
        ...
        dtn_log_error("Could not set time on timer");
        goto error;
    }
...
bool impl_event_loop_linux_timer_unset(dtn_event_loop *self,
    ...
    if (0 == loop) {
        dtn_log_error("called with wrong argument");
        goto error;
    }
